...
std::vector<bool> edge_split(
    AnnotatedNetwork &ann_network, const Edge *edge,
    std::unordered_map<std::string, unsigned int> &label_to_int, bool unrooted,
    bool softwired) {
...
std::vector<int> edge_trip(
    AnnotatedNetwork &ann_network, const Edge *edge,
    std::unordered_map<std::string, unsigned int> &label_to_int) {
  Network &network = ann_network.network;
  std::vector<unsigned int> descendant_in_trees_count(
      network.num_tips(), 0);  // first, we count in how many displayed trees a
                               // taxon is a descendant
...
int count_paths(AnnotatedNetwork &ann_network, const Node *source,
                const Node *target) {
  if (source == target) {
    return 1;
  }
...
std::vector<int> node_path_vector(
    AnnotatedNetwork &ann_network, Node *node,
    std::unordered_map<std::string, unsigned int> &label_to_int) {
  std::vector<int> path_vector(ann_network.network.num_tips(), 0);
  for (size_t i = 0; i < ann_network.network.num_tips(); ++i) {
    unsigned int tip_id =
        label_to_int[ann_network.network.nodes_by_index[i]->label];
    path_vector[tip_id] =
        count_paths(ann_network, node, ann_network.network.nodes_by_index[i]);
  }
  return path_vector;
}
...
bool is_trivial_trip(const std::vector<int> &trip) {
  unsigned int cnt_zeros = 0;  // these are the non-decendants of the edge
                               // associated with the given tripartition
  for (size_t i = 0; i < trip.size(); ++i) {
    if (trip[i] == 0) {
      cnt_zeros++;
    }
...
void add_splits(std::unordered_set<std::vector<bool>> &splits_hash,
                AnnotatedNetwork &ann_network,
                std::unordered_map<std::string, unsigned int> &label_to_int,
                bool unrooted, bool softwired) {
  for (size_t i = 0; i < ann_network.network.num_branches(); ++i) {
    std::vector<bool> act_split =
        edge_split(ann_network, ann_network.network.edges_by_index[i],
                   label_to_int, unrooted, softwired);
...
std::unordered_set<std::vector<bool>> extract_network_splits(
    AnnotatedNetwork &ann_network,
    std::unordered_map<std::string, unsigned int> &label_to_int, bool unrooted,
    bool softwired) {
...
struct NestedLabel {
  ...
  NestedLabel(int tip_id) { newick = std::to_string(tip_id); }

  bool empty() const { return newick.empty(); }
...
std::unordered_set<std::vector<int>, VectorHash> extract_network_trips(
    AnnotatedNetwork &ann_network,
    std::unordered_map<std::string, unsigned int> &label_to_int) {
  std::unordered_set<std::vector<int>, VectorHash> trips_hash;
  for (size_t i = 0; i < ann_network.network.num_branches(); ++i) {
    std::vector<int> act_trip = edge_trip(
        ann_network, ann_network.network.edges_by_index[i], label_to_int);
...
std::unordered_set<std::vector<int>, VectorHash> extract_network_path_vectors(
    AnnotatedNetwork &ann_network,
    std::unordered_map<std::string, unsigned int> &label_to_int) {
  std::unordered_set<std::vector<int>, VectorHash> path_vector_hash;
  for (size_t i = 0; i < ann_network.network.num_nodes(); ++i) {
    std::vector<int> act_path_vector = node_path_vector(
        ann_network, ann_network.network.nodes_by_index[i], label_to_int);
    path_vector_hash.emplace(act_path_vector);
  }
  return path_vector_hash;
}

void update_node_nested_labels(
    AnnotatedNetwork &ann_network, unsigned int node_idx,
    std::vector<NestedLabel> &nested_labels_by_node,
    std::unordered_map<std::string, unsigned int> &label_to_int) {
  ...
  if (node->isTip()) {
    ...
    for (auto &child : children) {
      if (nested_labels_by_node[child->clv_index].empty()) {
        update_node_nested_labels(ann_network, child->clv_index,
                                  nested_labels_by_node, label_to_int);
      }
...
std::unordered_set<NestedLabel, NestedLabelHash> extract_network_nested_labels(
    AnnotatedNetwork &ann_network,
    std::unordered_map<std::string, unsigned int> &label_to_int) {
  ...
  for (size_t i = 0; i < ann_network.network.num_nodes(); ++i) {
    update_node_nested_labels(ann_network, i, nested_labels_by_node,
                              label_to_int);
...
double rooted_tripartition_distance(
...
double cluster_distance(
    ...
    bool softwired) {
  auto splits_hash_1 =
      extract_network_splits(ann_network_1, label_to_int, unrooted, softwired);
  auto splits_hash_2 =
      extract_network_splits(ann_network_2, label_to_int, unrooted, softwired);
  return relative_distance_score(splits_hash_1, splits_hash_2);
}

double displayed_trees_distance(
    AnnotatedNetwork &ann_network_1, AnnotatedNetwork &ann_network_2,
    std::unordered_map<std::string, unsigned int> &label_to_int,
    bool unrooted) {
  ...
  for (int tree_index_1 = 0;
       tree_index_1 < 1 << ann_network_1.network.num_reticulations();
       ++tree_index_1) {
    netrax::setReticulationParents(ann_network_1.network, tree_index_1);
    add_splits(tree_splits_1[tree_index_1], ann_network_1, label_to_int,
               unrooted, true);
  }

  for (int tree_index_2 = 0;
       tree_index_2 < 1 << ann_network_2.network.num_reticulations();
       ++tree_index_2) {
    netrax::setReticulationParents(ann_network_2.network, tree_index_2);
    add_splits(tree_splits_2[tree_index_2], ann_network_2, label_to_int,
               unrooted, true);
  }

  unsigned int n_trees_both = 0;
  for (unsigned int tree_index_1 = 0; tree_index_1 < n_trees_1;
       ++tree_index_1) {
    bool found_equal_tree = false;
    for (unsigned int tree_index_2 = 0; tree_index_2 < n_trees_2;
         ++tree_index_2) {
      bool trees_equal =
          (count_in_both(tree_splits_1[tree_index_1],
                         tree_splits_2[tree_index_2]) == tree_splits_1.size());
      if (trees_equal) {
  ...
  double dist = (double)(n_trees_1 + n_trees_2 - 2 * n_trees_both) /
                (n_trees_1 + n_trees_2 - n_trees_both);
  return dist;
}

double rooted_path_multiplicity_distance(
    AnnotatedNetwork &ann_network_1, AnnotatedNetwork &ann_network_2,
    std::unordered_map<std::string, unsigned int> &label_to_int) {
...
double rooted_nested_labels_distance(
    AnnotatedNetwork &ann_network_1, AnnotatedNetwork &ann_network_2,
    std::unordered_map<std::string, unsigned int> &label_to_int) {
  auto labels_hash_1 =
      extract_network_nested_labels(ann_network_1, label_to_int);
  auto labels_hash_2 =
      extract_network_nested_labels(ann_network_2, label_to_int);
...
double get_network_distance(
  ...
  switch (type) {
    case NetworkDistanceType::UNROOTED_SOFTWIRED_DISTANCE:
      return cluster_distance(ann_network_1, ann_network_2, label_to_int, true,
                              true);
    case NetworkDistanceType::ROOTED_SOFTWIRED_DISTANCE:
      return cluster_distance(ann_network_1, ann_network_2, label_to_int, false,
                              true);
    ...
    case NetworkDistanceType::UNROOTED_DISPLAYED_TREES_DISTANCE:
      return displayed_trees_distance(ann_network_1, ann_network_2,
                                      label_to_int, true);
    case NetworkDistanceType::ROOTED_DISPLAYED_TREES_DISTANCE:
      return displayed_trees_distance(ann_network_1, ann_network_2,
                                      label_to_int, false);
    case NetworkDistanceType::ROOTED_TRIPARTITION_DISTANCE:
      return rooted_tripartition_distance(ann_network_1, ann_network_2,
                                          label_to_int);
    case NetworkDistanceType::ROOTED_PATH_MULTIPLICITY_DISTANCE:
      return rooted_path_multiplicity_distance(ann_network_1, ann_network_2,
                                               label_to_int);
    case NetworkDistanceType::ROOTED_NESTED_LABELS_DISTANCE:
      return rooted_nested_labels_distance(ann_network_1, ann_network_2,
                                           label_to_int);
    default:
      throw std::runtime_error(
          "Required network distance type not implemented yet!");
  }
}
}  // namespace netrax