...
public:
	...
	pllmod_treeinfo_t* createNetworkPllTreeinfo(Network &network, unsigned int tips, unsigned int partitions, int brlen_linkage);
	...
	double network_opt_brlen_wrapper(pllmod_treeinfo_t *fake_treeinfo, double min_brlen, double max_brlen, double lh_epsilon, int max_iters,
	...
	double network_spr_round_wrapper(pllmod_treeinfo_t *treeinfo, unsigned int radius_min, unsigned int radius_max,
			unsigned int ntopol_keep, pll_bool_t thorough, int brlen_opt_method, double bl_min, double bl_max, int smoothings,
	...
	void network_init_treeinfo_wrapper(const Options &opts, const std::vector<doubleVector> &partition_brlens, size_t num_branches,
			...
			IDSet *parts_master);
	static void network_create_init_partition_wrapper(size_t p, int params_to_optimize, pllmod_treeinfo_t* pll_treeinfo,
