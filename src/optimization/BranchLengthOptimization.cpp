...
double optimize_branch_newton_raphson(AnnotatedNetwork &ann_network, std::vector<std::vector<SumtableInfo> >& sumtables, std::vector<DisplayedTreeData>& oldTrees, size_t pmatrix_index, size_t partition_index, unsigned int max_iters) {
    double old_brlen;
    if (ann_network.fake_treeinfo->brlen_linkage == PLLMOD_COMMON_BRLEN_UNLINKED) {
        old_brlen = ann_network.fake_treeinfo->branch_lengths[partition_index][pmatrix_index];
    } else {
...
double optimize_branch(AnnotatedNetwork &ann_network, size_t pmatrix_index, BrlenOptMethod brlenOptMethod, unsigned int max_iters) {
    ...
    if (brlenOptMethod != BrlenOptMethod::BRENT_NORMAL) {
        ...
        if (fabs(old_logl - brlenopt_logl >= 1E-3)) {
            ...
            double nonincremental_logl = computeLoglikelihoodBrlenOpt(ann_network, oldTrees, pmatrix_index, 0, 1, true);
            if (ParallelContext::master_rank() && ParallelContext::master_thread) {
                std::cout << "nonincremental_logl: " << nonincremental_logl << "\n";
            }
