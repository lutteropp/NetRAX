...
struct BrentBrlenParams {
    ...
    std::vector<TreeLoglData>* oldTrees = nullptr;
...
static double brent_target_networks(void *p, double x) {
    ...
    std::vector<TreeLoglData>* oldTrees = ((BrentBrlenParams*) p)->oldTrees;
...
double optimize_branch_brent(AnnotatedNetwork &ann_network, std::vector<TreeLoglData>& oldTrees, std::vector<std::vector<SumtableInfo> >& sumtables, size_t pmatrix_index, size_t partition_index, BrlenOptMethod brlenOptMethod) {
                                         ...
                                         double *df, double *ddf)
{
  ...
  LoglDerivatives logl_derivatives = computeLoglikelihoodDerivatives(*ann_network, *(params->sumtables), params->pmatrix_index, 1, 1);
...
double optimize_branch_newton_raphson(AnnotatedNetwork &ann_network, std::vector<std::vector<SumtableInfo> >& sumtables, std::vector<TreeLoglData>& oldTrees, size_t pmatrix_index, size_t partition_index, BrlenOptMethod brlenOptMethod, unsigned int max_iters) {
...
double optimize_branch(AnnotatedNetwork &ann_network, std::vector<TreeLoglData>& oldTrees, std::vector<std::vector<SumtableInfo> >& sumtables, size_t pmatrix_index, size_t partition_index, BrlenOptMethod brlenOptMethod, unsigned int max_iters) {
...
double optimize_branches(AnnotatedNetwork &ann_network, int max_iters, int max_iters_outside, int radius, bool restricted_total_iters) {
