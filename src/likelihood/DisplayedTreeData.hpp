...
struct TreeLoglData {
    ...
    TreeLoglData(size_t n_partitions, size_t max_reticulations) : reticulationChoices(max_reticulations) {
        ...
        ReticulationConfig allConfig(max_reticulations);
        allConfig.config = std::vector<ReticulationState>(max_reticulations, ReticulationState::DONT_CARE);
        reticulationChoices.configs.emplace_back(allConfig);
    }
