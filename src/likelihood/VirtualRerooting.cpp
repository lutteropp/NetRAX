...
struct PathToVirtualRoot {
  ...
  PathToVirtualRoot(size_t max_reticulations)
      : reticulationChoices(max_reticulations){};
};

std::vector<Node *> getPathToVirtualRoot(Node *from, Node *virtual_root,
...
void printPathToVirtualRoot(const PathToVirtualRoot &pathToVirtualRoot) {
  ...
  for (size_t i = 0; i < pathToVirtualRoot.path.size(); ++i) {
    std::cout << "Node " << pathToVirtualRoot.path[i]->clv_index
              << " has children: ";
...
std::vector<PathToVirtualRoot> getPathsToVirtualRoot(
    AnnotatedNetwork &ann_network, Node *old_virtual_root,
    Node *new_virtual_root, Node *new_virtual_root_back) {
  std::vector<PathToVirtualRoot> res;
  // naive version here: Go over all displayed trees, compute pathToVirtualRoot
  // for each of them, and then later on kick out duplicate paths...

  NodeDisplayedTreeData &oldDisplayedTrees =
      ann_network.pernode_displayed_tree_data[old_virtual_root->clv_index];
  for (size_t i = 0; i < oldDisplayedTrees.num_active_displayed_trees; ++i) {
    ...
    for (size_t partition_idx = 0;
         ...
         ++partition_idx) {
      setReticulationParents(ann_network.network,
                             oldDisplayedTrees.displayed_trees[i]
                                 .treeLoglData.reticulationChoices.configs[0]);
    }
    std::vector<Node *> parent =
        getParentPointers(ann_network, new_virtual_root);
    std::vector<Node *> path =
        getPathToVirtualRoot(old_virtual_root, new_virtual_root, parent);

    std::vector<ReticulationState> dont_care_vector(
    ...
    for (size_t j = 0; j < path.size() - 1; ++j) {
      restrictionsSet = combineReticulationChoices(
          restrictionsSet,
          getRestrictionsToTakeNeighbor(ann_network, path[j], path[j + 1]));
    }
    ...
    for (size_t j = 0; j < path.size() - 1; ++j) {
      if (path[j] == new_virtual_root_back) {
        ptvr.children.emplace_back(getCurrentChildren(
            ann_network, path[j], new_virtual_root,
            restrictionsSet));  // Not sure if this special case is needed
      } else {
        ptvr.children.emplace_back(getCurrentChildren(
            ann_network, path[j], parent[path[j]->clv_index], restrictionsSet));
      }
    }
    // Special case at new virtual root, there we don't want to include
    // new_virtual_root_back...
    assert(path[path.size() - 1] == new_virtual_root);
    ptvr.children.emplace_back(getCurrentChildren(
        ann_network, new_virtual_root, new_virtual_root_back, restrictionsSet));
...
NodeSaveInformation computeNodeSaveInformation(
    const std::vector<PathToVirtualRoot> &paths) {
  NodeSaveInformation nodeSaveInfo;
  std::vector<std::unordered_set<size_t>> &pathNodesToRestore =
      nodeSaveInfo.pathNodesToRestore;
  ...
  for (size_t p = 1; p < paths.size(); ++p) {
    ...
    // check if the node occurs at an earlier path. If so, it really needs to be
    // restored, as that earlier path would have overwritten it.
...
void updateCLVsVirtualRerootTrees(AnnotatedNetwork &ann_network,
  ...
  //     restrictions storing the reticulation configurations used 1.2) update
  //     CLVs on that path, using extra restrictions and append mode
  std::vector<PathToVirtualRoot> paths = getPathsToVirtualRoot(
      ann_network, old_virtual_root, new_virtual_root, new_virtual_root_back);
  ...
  std::vector<NodeDisplayedTreeData> bufferedNodeInformations(
      ann_network.network.num_nodes());
  for (size_t partition_idx = 0;
       partition_idx < ann_network.fake_treeinfo->partition_count;
       ++partition_idx) {
    for (size_t nodeInDanger : nodeSaveInfo.nodesInDanger) {
      bufferedNodeInformations[nodeInDanger] =
          ann_network.pernode_displayed_tree_data[nodeInDanger];
    }
  }
  ...
  for (size_t p = 0; p < paths.size(); ++p) {
    ...
    for (size_t i = 0; i < paths[p].path.size(); ++i) {
      ...
      assert((paths[p].path[i] != new_virtual_root) ||
             ((paths[p].path[i] == new_virtual_root) &&
              (i == paths[p].path.size() - 1)));
      processNodeImproved(ann_network, 0, paths[p].path[i],
                          paths[p].children[i], paths[p].reticulationChoices,
                          appendMode);
    }
  }
...
void updateTreeData(AnnotatedNetwork &ann_network,
                    const std::vector<DisplayedTreeData> &oldTrees,
...
void recomputeTreeData(AnnotatedNetwork &ann_network, size_t pmatrix_index,
  ...
  for (size_t p = 0; p < ann_network.fake_treeinfo->partition_count; ++p) {
    ...
    std::vector<double> persite_logl(
        ann_network.fake_treeinfo->partitions[p]->sites);
    assert(sourceTree.isTip ||
           !single_clv_is_all_zeros(ann_network.partition_clv_ranges[p],
                                    sourceTree.clv_vector[p]));
    assert(targetTree.isTip ||
           !single_clv_is_all_zeros(ann_network.partition_clv_ranges[p],
                                    targetTree.clv_vector[p]));
    combinedTreeData.tree_partition_logl[p] = pll_compute_edge_loglikelihood(
        partition, source->clv_index, sourceTree.clv_vector[p],
        sourceTree.scale_buffer[p], target->clv_index, targetTree.clv_vector[p],
        targetTree.scale_buffer[p], pmatrix_index,
        ann_network.fake_treeinfo->param_indices[p], persite_logl.data());
    assert(combinedTreeData.tree_partition_logl[p] !=
...
double computeLoglikelihoodBrlenOpt(
    ...
    int update_pmatrices, bool print_extra_debug_info) {
  ...
  Node *target = getTarget(ann_network.network,
                           ann_network.network.edges_by_index[pmatrix_index]);
  NodeDisplayedTreeData &sourceData =
      ann_network.pernode_displayed_tree_data[source->clv_index];
  NodeDisplayedTreeData &targetData =
  ...
  if (!clvValidCheck(ann_network, ann_network.network.root->clv_index, false)) {
    ...
    throw std::runtime_error(
        "Cannot reuse old displayed trees. For some reason, they are "
        "invalidated at the root node " +
        std::to_string(ann_network.network.root->clv_index));
  }
  if (update_pmatrices) {
      ...
      if (isActiveAliveBranch(ann_network, combinedTreeData.reticulationChoices,
        ...
        if (print_extra_debug_info) {
          if (ParallelContext::master_rank() &&
            ...
            ReticulationConfigSet restrictions =
                getRestrictionsToTakeNeighbor(ann_network, source, target);
  ...
  for (size_t i = 0; i < oldTrees.size(); ++i) {
    ...
    for (size_t j = 0; j < combinedTrees.size(); ++j) {
      if (reticulationConfigsCompatible(
              ...
              combinedTrees[j].reticulationChoices)) {
        seen = true;
        if (print_extra_debug_info) {
          if (ParallelContext::master_rank() &&
              ParallelContext::master_thread()) {
  ...
  // find trees that are still not present in the combined trees, but may be
  // partially present in the old trees
  for (size_t tree_idx = 0;
    ...
    for (size_t i = 0; i < combinedTrees.size(); ++i) {
      if (reticulationConfigsCompatible(treeChoices,
                                        combinedTrees[i].reticulationChoices)) {
        seen = true;
        break;
      }
    }
    ...
    if (!seen) {
      ...
      for (size_t i = 0; i < oldTrees.size(); ++i) {
        if (reticulationConfigsCompatible(
                treeChoices, oldTrees[i].treeLoglData.reticulationChoices)) {
          TreeLoglData combinedTreeData(
