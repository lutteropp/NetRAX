...
double displayed_tree_logprob(AnnotatedNetwork &ann_network,
                              size_t tree_index) {
  ...
  for (size_t i = 0; i < network.num_reticulations(); ++i) {
    mpfr::mpreal prob =
        getReticulationActiveProb(ann_network, network.reticulation_nodes[i]);
    if (prob == 0.0) {
...
/**
 * Compute network loglikelihood by converting each displayed tree into a
 *pll_utree_t and calling raxml loglikelihood on it. Then taking the weighted
 *sum of the displayed tree likelihoods (using exp) for the network likelihood,
 *and returning th log of the network likelihood. Using arbitrary-precision
 *floating point operations for aggregating over the displayed tree
 *loglikelihoods. This naive implementation assumes a single partition in the
 *MSA. It also doesn't do model optimization.
 **/
double computeLoglikelihoodNaiveUtree(AnnotatedNetwork &ann_network,
  ...
  // ensure the network edge lengths are up-to-date (needed for displayed tree
  // to utree)
  for (size_t i = 0; i < ann_network.network.num_branches(); ++i) {
    ann_network.network.edges_by_index[i]->length =
        ann_network.fake_treeinfo->branch_lengths[0][i];
  }

  std::vector<std::vector<double>> tree_logl_per_partition(
      num_partitions,
      std::vector<double>(num_trees, -std::numeric_limits<double>::infinity()));
  ...
  for (size_t i = 0; i < num_trees; ++i) {
    ...
    TreeInfo *displayedTreeinfo =
        createRaxmlTreeinfo(displayed_tree, ann_network.instance);
    ...
    for (size_t p = 0; p < num_partitions; ++p) {
      tree_logl_per_partition[p][i] =
          displayedTreeinfo->pll_treeinfo().partition_loglh[p];
    }
  ...
  for (size_t partition_idx = 0; partition_idx < num_partitions;
       ++partition_idx) {
    ...
    if (ann_network.options.likelihood_variant ==
      ...
      for (size_t i = 0; i < num_trees; ++i) {
        if (tree_logprob[i] != std::numeric_limits<double>::infinity()) {
          partition_lh += mpfr::exp(tree_logl_per_partition[partition_idx][i]) *
                          mpfr::exp(tree_logprob[i]);
        }
      }
      ...
      for (size_t i = 0; i < num_trees; ++i) {
        if (tree_logprob[i] != std::numeric_limits<double>::infinity()) {
          partition_logl = std::max(
              partition_logl,
              tree_logl_per_partition[partition_idx][i] + tree_logprob[i]);
        }
      }
    }
