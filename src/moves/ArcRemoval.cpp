...
bool checkSanityArcRemoval(AnnotatedNetwork &ann_network, const Move &move) {
  bool good = true;
  good &= (move.moveType == MoveType::ArcRemovalMove ||
           move.moveType == MoveType::DeltaMinusMove);
  good &=
      (ann_network.network.nodes_by_index[move.arcRemovalData.a_clv_index] !=
       nullptr);
  good &=
      (ann_network.network.nodes_by_index[move.arcRemovalData.b_clv_index] !=
       nullptr);
  good &=
      (ann_network.network.nodes_by_index[move.arcRemovalData.c_clv_index] !=
       nullptr);
  good &=
      (ann_network.network.nodes_by_index[move.arcRemovalData.d_clv_index] !=
       nullptr);
  good &=
      (ann_network.network.nodes_by_index[move.arcRemovalData.u_clv_index] !=
       nullptr);
  good &=
      (ann_network.network.nodes_by_index[move.arcRemovalData.v_clv_index] !=
       nullptr);
  good &=
      (ann_network.network
           .edges_by_index[move.arcRemovalData.au_pmatrix_index] != nullptr);
  good &=
      (ann_network.network
           .edges_by_index[move.arcRemovalData.ub_pmatrix_index] != nullptr);
  good &=
      (ann_network.network
           .edges_by_index[move.arcRemovalData.uv_pmatrix_index] != nullptr);
  good &=
      (ann_network.network
           .edges_by_index[move.arcRemovalData.cv_pmatrix_index] != nullptr);
  good &=
      (ann_network.network
           .edges_by_index[move.arcRemovalData.vd_pmatrix_index] != nullptr);
...
bool checkSanityArcRemoval(AnnotatedNetwork &ann_network,
...
void updateMoveClvIndex(Move &move, size_t old_clv_index,
                        size_t new_clv_index) {
  Move origMove = move;
  // set new to old
...
Move buildMoveArcRemoval(
  ...
  move.arcRemovalData.v_first_parent_clv_index = v_first_parent_clv_index;
  ...
  std::string u_label =
      ann_network.network.nodes_by_index[u_clv_index]->getLabel();
  std::string v_label =
      ann_network.network.nodes_by_index[v_clv_index]->getLabel();
  std::string u_reticulation_label;
  if (ann_network.network.nodes_by_index[u_clv_index]->getType() ==
      NodeType::RETICULATION_NODE) {
    u_reticulation_label = ann_network.network.nodes_by_index[u_clv_index]
                               ->getReticulationData()
                               ->getLabel();
  }
  std::string v_reticulation_label;
  if (ann_network.network.nodes_by_index[v_clv_index]->getType() ==
      NodeType::RETICULATION_NODE) {
    v_reticulation_label = ann_network.network.nodes_by_index[v_clv_index]
                               ->getReticulationData()
                               ->getLabel();
  }
...
std::vector<Move> possibleMovesArcRemoval(AnnotatedNetwork &ann_network,
                                          ...
                                          MoveType moveType) {
  // v is a reticulation node, u is one parent of v, c is the other parent of v,
  // a is parent of u, d is child of v, b is other child of u
  std::vector<Move> res;
  ...
  if (first_parent != network.root &&
      first_parent->type != NodeType::RETICULATION_NODE) {
    ucChoices.emplace_back(std::make_pair(first_parent, second_parent));
  }
  if (second_parent != network.root &&
      second_parent->type != NodeType::RETICULATION_NODE) {
  ...
  for (size_t i = 0; i < ucChoices.size(); ++i) {
    ...
    std::vector<double> a_u_len, u_b_len, a_b_len, c_v_len, v_d_len, c_d_len,
        u_v_len;
    a_u_len =
        get_edge_lengths(ann_network, getEdgeTo(network, a, u)->pmatrix_index);
    u_b_len =
        get_edge_lengths(ann_network, getEdgeTo(network, u, b)->pmatrix_index);
    a_b_len = get_plus_edge_lengths(ann_network, a_u_len, u_b_len, max_br);
    c_v_len =
        get_edge_lengths(ann_network, getEdgeTo(network, c, v)->pmatrix_index);
    v_d_len =
        get_edge_lengths(ann_network, getEdgeTo(network, v, d)->pmatrix_index);
    c_d_len = get_plus_edge_lengths(ann_network, c_v_len, v_d_len, max_br);
    u_v_len =
        get_edge_lengths(ann_network, getEdgeTo(network, u, v)->pmatrix_index);

    Move move = buildMoveArcRemoval(
        ann_network, a->clv_index, b->clv_index, c->clv_index, d->clv_index,
        u->clv_index, v->clv_index, getReticulationFirstParent(ann_network.network, v)->clv_index, u_v_len, c_v_len, a_u_len, a_b_len, c_d_len,
        v_d_len, u_b_len, moveType, edge_orig_idx, v->clv_index);

    move.arcRemovalData.au_pmatrix_index =
        getEdgeTo(network, a, u)->pmatrix_index;
    move.arcRemovalData.ub_pmatrix_index =
        getEdgeTo(network, u, b)->pmatrix_index;
    move.arcRemovalData.cv_pmatrix_index =
        getEdgeTo(network, c, v)->pmatrix_index;
    move.arcRemovalData.vd_pmatrix_index =
        getEdgeTo(network, v, d)->pmatrix_index;
    move.arcRemovalData.uv_pmatrix_index =
...
std::vector<Move> possibleMovesDeltaMinus(AnnotatedNetwork &ann_network,
                                          Node *v, size_t edge_orig_idx) {
  std::vector<Move> res;
  std::vector<Move> allRemovals = possibleMovesArcRemoval(
      ann_network, v, edge_orig_idx, MoveType::DeltaMinusMove);
  // 3 cases: a == c, b == d, or b == c
  for (size_t i = 0; i < allRemovals.size(); ++i) {
    if ((allRemovals[i].arcRemovalData.a_clv_index ==
         allRemovals[i].arcRemovalData.c_clv_index) ||
        (allRemovals[i].arcRemovalData.b_clv_index ==
         allRemovals[i].arcRemovalData.d_clv_index) ||
        (allRemovals[i].arcRemovalData.b_clv_index ==
         allRemovals[i].arcRemovalData.c_clv_index)) {
...
std::vector<Move> possibleMovesArcRemoval(AnnotatedNetwork &ann_network) {
  ...
  for (size_t i = 0; i < network.num_reticulations(); ++i) {
    size_t edge_orig_idx = getReticulationFirstParentPmatrixIndex(
        ann_network.network.reticulation_nodes[i]);
    auto moves =
        possibleMovesArcRemoval(ann_network, network.reticulation_nodes[i],
                                edge_orig_idx, MoveType::ArcRemovalMove);
...
std::vector<Move> possibleMovesDeltaMinus(AnnotatedNetwork &ann_network) {
  ...
  for (size_t i = 0; i < network.num_reticulations(); ++i) {
    size_t edge_orig_idx = getReticulationFirstParentPmatrixIndex(
        ann_network.network.reticulation_nodes[i]);
    std::vector<Move> branch_moves = possibleMovesDeltaMinus(
        ann_network, network.reticulation_nodes[i], edge_orig_idx);
...
void updateMoveClvIndexArcRemoval(Move &move, size_t old_clv_index,
  ...
  if (move.arcRemovalData.v_first_parent_clv_index == old_clv_index) {
    move.arcRemovalData.v_first_parent_clv_index = new_clv_index;
  } else if (move.arcRemovalData.v_first_parent_clv_index == new_clv_index) {
    move.arcRemovalData.v_first_parent_clv_index = old_clv_index;
  }
}
...
void repairConsecutiveClvIndices(AnnotatedNetwork &ann_network, Move &move) {
  ...
  for (size_t i = 0; i < ann_network.network.nodes.size(); ++i) {
    if (ann_network.network.nodes[i].clv_index >=
            ann_network.network.num_nodes() &&
        ann_network.network.nodes[i].clv_index <
...
void repairConsecutivePmatrixIndices(AnnotatedNetwork &ann_network,
                                     Move &move) {
  ...
  for (size_t i = 0; i < ann_network.network.edges.size(); ++i) {
    if (ann_network.network.edges[i].pmatrix_index >=
            ann_network.network.num_branches() &&
        ann_network.network.edges[i].pmatrix_index <
            std::numeric_limits<size_t>::max()) {
      size_t old_pmatrix_index = ann_network.network.edges[i].pmatrix_index;
      size_t new_pmatrix_index = missing_pmatrix_indices.back();
      swapPmatrixIndex(ann_network, move, old_pmatrix_index, new_pmatrix_index,
                       false);
...
// TODO: This relabeling procedure will invalidate remaining arc removal move
candidates. This is why we need to undo this remapping afterwards.
// ensure that pmatrix indices and clv indices remain consecutive. Do the
neccessary relabelings. repairConsecutiveClvIndices(ann_network, move);
repairConsecutivePmatrixIndices(ann_network, move);
}*/

bool isReticulation(AnnotatedNetwork &ann_network, size_t clvIdx) {
  auto it =
      std::find_if(ann_network.network.reticulation_nodes.begin(),
                   ann_network.network.reticulation_nodes.end(),
                   [&clvIdx](Node *node) { return node->clv_index == clvIdx; });
...
void performMoveArcRemoval(AnnotatedNetwork &ann_network, Move &move) {
  ...
  for (size_t k = 0; k <
ann_network.pernode_displayed_tree_data[ann_network.network.root->clv_index].num_active_displayed_trees;
++k) { ReticulationConfigSet& rcs =
ann_network.pernode_displayed_tree_data[ann_network.network.root->clv_index].displayed_trees[k].treeLoglData.reticulationChoices;
  ...
  assert(move.moveType == MoveType::ArcRemovalMove ||
         move.moveType == MoveType::DeltaMinusMove);
  assert(assertConsecutiveIndices(ann_network));

  Network &network = ann_network.network;
  Link *from_a_link = getLinkToNode(network, move.arcRemovalData.a_clv_index,
                                    move.arcRemovalData.u_clv_index);
  Link *to_b_link = getLinkToNode(network, move.arcRemovalData.b_clv_index,
                                  move.arcRemovalData.u_clv_index);
  Link *from_c_link = getLinkToNode(network, move.arcRemovalData.c_clv_index,
                                    move.arcRemovalData.v_clv_index);
  Link *to_d_link = getLinkToNode(network, move.arcRemovalData.d_clv_index,
                                  move.arcRemovalData.v_clv_index);
  assert(from_a_link);
  assert(to_b_link);
  assert(from_c_link);
  ...
  assert(getEdgeTo(network, move.arcRemovalData.a_clv_index,
                   move.arcRemovalData.u_clv_index));
  assert(getEdgeTo(network, move.arcRemovalData.u_clv_index,
                   move.arcRemovalData.b_clv_index));
  assert(getEdgeTo(network, move.arcRemovalData.c_clv_index,
                   move.arcRemovalData.v_clv_index));
  assert(getEdgeTo(network, move.arcRemovalData.v_clv_index,
                   move.arcRemovalData.d_clv_index));
  assert(getEdgeTo(network, move.arcRemovalData.u_clv_index,
                   move.arcRemovalData.v_clv_index));
  ...
  assert(getEdgeTo(network, move.arcRemovalData.a_clv_index,
                   move.arcRemovalData.u_clv_index)
             ->pmatrix_index == move.arcRemovalData.au_pmatrix_index);
  removeEdge(ann_network, move,
             network.edges_by_index[move.arcRemovalData.au_pmatrix_index],
             false);
  assert(getEdgeTo(network, move.arcRemovalData.u_clv_index,
                   move.arcRemovalData.b_clv_index)
             ->pmatrix_index == move.arcRemovalData.ub_pmatrix_index);
  removeEdge(ann_network, move,
             network.edges_by_index[move.arcRemovalData.ub_pmatrix_index],
             false);
  assert(getEdgeTo(network, move.arcRemovalData.c_clv_index,
                   move.arcRemovalData.v_clv_index)
             ->pmatrix_index == move.arcRemovalData.cv_pmatrix_index);
  removeEdge(ann_network, move,
             network.edges_by_index[move.arcRemovalData.cv_pmatrix_index],
             false);
  assert(getEdgeTo(network, move.arcRemovalData.v_clv_index,
                   move.arcRemovalData.d_clv_index)
             ->pmatrix_index == move.arcRemovalData.vd_pmatrix_index);
  removeEdge(ann_network, move,
             network.edges_by_index[move.arcRemovalData.vd_pmatrix_index],
             false);
  assert(getEdgeTo(network, move.arcRemovalData.u_clv_index,
                   move.arcRemovalData.v_clv_index)
             ->pmatrix_index == move.arcRemovalData.uv_pmatrix_index);
  removeEdge(ann_network, move,
             network.edges_by_index[move.arcRemovalData.uv_pmatrix_index],
             false);

  for (size_t i = 0; i < network.num_reticulations(); ++i) {
    assert(network.reticulation_nodes[i]->type == NodeType::RETICULATION_NODE);
  }
  assert(network.nodes_by_index[move.arcRemovalData.u_clv_index]->clv_index ==
         move.arcRemovalData.u_clv_index);
  removeNode(ann_network, move,
             network.nodes_by_index[move.arcRemovalData.u_clv_index], false);
  assert(network.nodes_by_index[move.arcRemovalData.v_clv_index]->clv_index ==
         move.arcRemovalData.v_clv_index);
  removeNode(ann_network, move,
             network.nodes_by_index[move.arcRemovalData.v_clv_index], false);
  for (size_t i = 0; i < network.num_reticulations(); ++i) {
    assert(network.reticulation_nodes[i]->type == NodeType::RETICULATION_NODE);
  }
  ...
  Edge *a_b_edge =
      addEdge(ann_network, from_a_link, to_b_link, a_b_edge_length[0],
              move.arcRemovalData.wanted_ab_pmatrix_index);
  Edge *c_d_edge =
      addEdge(ann_network, from_c_link, to_d_link, c_d_edge_length[0],
  ...
  assert(ann_network.network.nodes_by_index[move.arcRemovalData.u_clv_index] ==
         nullptr);
  assert(ann_network.network.nodes_by_index[move.arcRemovalData.v_clv_index] ==
         nullptr);
  ...
  if (b->type == NodeType::RETICULATION_NODE) {
    ...
    if (getReticulationFirstParentPmatrixIndex(b) ==
        move.arcRemovalData.ub_pmatrix_index) {
      badToParentLink = b->getReticulationData()->link_to_first_parent;
    } else {
      assert(getReticulationSecondParentPmatrixIndex(b) ==
             move.arcRemovalData.ub_pmatrix_index);
  ...
  if (d->type == NodeType::RETICULATION_NODE) {
    ...
    if (getReticulationFirstParentPmatrixIndex(d) ==
        move.arcRemovalData.vd_pmatrix_index) {
      badToParentLink = d->getReticulationData()->link_to_first_parent;
    } else {
      assert(getReticulationSecondParentPmatrixIndex(d) ==
             move.arcRemovalData.vd_pmatrix_index);
  ...
  invalidateHigherCLVs(ann_network,
                       ...
                       false, visited);
  invalidateHigherCLVs(ann_network,
                       network.nodes_by_index[move.arcRemovalData.b_clv_index],
                       false, visited);
  invalidatePmatrixIndex(ann_network, a_b_edge->pmatrix_index, visited);
  invalidateHigherCLVs(ann_network,
                       network.nodes_by_index[move.arcRemovalData.c_clv_index],
                       false, visited);
  invalidateHigherCLVs(ann_network,
                       network.nodes_by_index[move.arcRemovalData.d_clv_index],
                       false, visited);
  ...
  for (size_t i = 0; i < ann_network.fake_treeinfo->partition_count; ++i) {
    ...
    all_clvs_valid &= ann_network.fake_treeinfo
                          ->clv_valid[i][ann_network.network.root->clv_index];
  }
...
void undoMoveArcRemoval(AnnotatedNetwork &ann_network, Move &move) {
  ...
  Move insertion = buildMoveArcInsertion(
      ann_network, move.arcRemovalData.a_clv_index,
      move.arcRemovalData.b_clv_index, move.arcRemovalData.c_clv_index,
      move.arcRemovalData.d_clv_index, move.arcRemovalData.u_v_len,
      move.arcRemovalData.c_v_len, move.arcRemovalData.a_u_len,
      move.arcRemovalData.a_b_len, move.arcRemovalData.c_d_len,
      move.arcRemovalData.v_d_len, move.arcRemovalData.u_b_len,
      MoveType::ArcInsertionMove, move.edge_orig_idx, move.node_orig_idx);

  insertion.arcInsertionData.wanted_u_clv_index =
      move.arcRemovalData.u_clv_index;
  insertion.arcInsertionData.wanted_v_clv_index =
      move.arcRemovalData.v_clv_index;

  insertion.arcInsertionData.wanted_au_pmatrix_index =
      move.arcRemovalData.au_pmatrix_index;
  insertion.arcInsertionData.wanted_ub_pmatrix_index =
      move.arcRemovalData.ub_pmatrix_index;
  insertion.arcInsertionData.wanted_cv_pmatrix_index =
      move.arcRemovalData.cv_pmatrix_index;
  insertion.arcInsertionData.wanted_vd_pmatrix_index =
      move.arcRemovalData.vd_pmatrix_index;
  insertion.arcInsertionData.wanted_uv_pmatrix_index =
      move.arcRemovalData.uv_pmatrix_index;

  insertion.arcInsertionData.ab_pmatrix_index =
      move.arcRemovalData.wanted_ab_pmatrix_index;
  insertion.arcInsertionData.cd_pmatrix_index =
      move.arcRemovalData.wanted_cd_pmatrix_index;
  ...
  // fix node labels
  ann_network.network.nodes_by_index[move.arcRemovalData.u_clv_index]->label =
      move.arcRemovalData.u_label;
  ann_network.network.nodes_by_index[move.arcRemovalData.v_clv_index]->label =
      move.arcRemovalData.v_label;
  if (isReticulation(ann_network, move.arcRemovalData.u_clv_index)) {
    ann_network.network.nodes_by_index[move.arcRemovalData.u_clv_index]
        ->getReticulationData()
        ->label = move.arcRemovalData.u_reticulation_label;
  }
  ...
  assert(v);
  assert(v->getType() == NodeType::RETICULATION_NODE);
  if (getReticulationFirstParent(ann_network.network, v)->clv_index != move.arcRemovalData.v_first_parent_clv_index) {
    assert(getReticulationSecondParent(ann_network.network, v)->clv_index == move.arcRemovalData.v_first_parent_clv_index);
...
std::string toStringArcRemoval(const Move &move) {
  ...
  ss << "  v_first_parent_clv_index = " << move.arcRemovalData.v_first_parent_clv_index << "\n";
