...
std::vector<Node *> getActiveNeighbors(Network &network, const Node *node) {
  ...
  for (size_t i = 0; i < neighbors.size(); ++i) {
    if (neighbors[i]->getType() == NodeType::RETICULATION_NODE) {
      // we need to check if the neighbor is active, this is, if we are
      // currently the selected parent
...
std::unordered_set<size_t> getNeighborPmatrixIndices(Network &network,
  ...
  assert(res.size() == 2 || res.size() == 4 ||
         (source == network.root && res.size() == 3) ||
...
bool topology_equal(Network &n1, Network &n2) {
  ...
  for (size_t i = 0; i < n1.num_branches(); ++i) {
    if (getSource(n1, n1.edges_by_index[i])->clv_index !=
        getSource(n2, n2.edges_by_index[i])->clv_index) {
      std::cout << "topology not equal\n";
      return false;
    }
    if (getTarget(n1, n1.edges_by_index[i])->clv_index !=
        getTarget(n2, n2.edges_by_index[i])->clv_index) {
      std::cout << "topology not equal\n";
      return false;
    }
  }
