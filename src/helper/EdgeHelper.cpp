...
Edge *getEdgeTo(Network &network, const Node *node, const Node *target) {
  ...
  throw std::runtime_error(
      "The given target node is not a neighbor of this node");
}
...
bool isActiveBranch(AnnotatedNetwork &ann_network,
  ...
  const Node *edge_source = getSource(
      ann_network.network, ann_network.network.edges_by_index[pmatrix_index]);
  const Node *edge_target = getTarget(
      ann_network.network, ann_network.network.edges_by_index[pmatrix_index]);

  ReticulationConfigSet restrictions =
      getRestrictionsToTakeNeighbor(ann_network, edge_source, edge_target);
...
bool isActiveAliveBranch(AnnotatedNetwork &ann_network,
  ...
  ReticulationConfigSet restrictions =
      getRestrictionsToTakeNeighbor(ann_network, edge_source, edge_target);
  return reticulationConfigsCompatible(restrictions, reticulationChoices) &&
         !dead_nodes[edge_source->clv_index] &&
         !dead_nodes[edge_target->clv_index];
}
...
bool isActiveAliveBranch(AnnotatedNetwork &ann_network,
  ...
  return isActiveAliveBranch(ann_network, reticulationChoices, dead_nodes,
                             pmatrix_index);
}
