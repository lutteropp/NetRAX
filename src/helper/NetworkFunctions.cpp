...
void remove_dead_children(std::vector<Node *> &children,
                          const std::vector<bool> &dead_nodes) {
...
std::vector<Node *> getChildrenNoDir(Network &network, const Node *node,
                                     const Node *myParent) {
  assert(node);
...
std::vector<Node *> getActiveChildrenNoDir(Network &network, const Node *node,
                                           const Node *myParent) {
  assert(node);
  ...
  for (size_t i = 0; i < children.size(); ++i) {
    if (children[i]->getType() == NodeType::RETICULATION_NODE) {
      // we need to check if the child is active, this is, if we are currently
      // the selected parent
  ...
  assert(activeChildren.size() <= 2 ||
         (myParent == nullptr && activeChildren.size() == 3));
  return activeChildren;
}

CumulatedChild getCumulatedChild(Network &network, const Node *parent,
                                 const Node *child,
                                 const std::vector<bool> &dead_nodes,
  ...
  while (skipped_nodes[res.child->clv_index]) {
    std::vector<Node *> activeChildren =
        getActiveChildrenNoDir(network, res.child, act_parent);
...
std::vector<CumulatedChild> getCumulatedChildren(
    Network &network, const Node *parent, const Node *actNode,
    const std::vector<bool> &dead_nodes,
    const std::vector<bool> &skipped_nodes) {
  assert(actNode);
  ...
  std::vector<Node *> activeChildren =
      getActiveChildrenNoDir(network, actNode, parent);
  for (size_t i = 0; i < activeChildren.size(); ++i) {
    res.push_back(getCumulatedChild(network, actNode, activeChildren[i],
                                    dead_nodes, skipped_nodes));
  }
  return res;
}

pll_unode_t *connect_subtree_recursive(Network &network,
                                       const Node *networkNode,
                                       pll_unode_t *from_parent,
                                       const Node *networkParentNode,
                                       const std::vector<bool> &dead_nodes,
                                       const std::vector<bool> &skipped_nodes) {
  assert(networkNode->getType() == NodeType::BASIC_NODE);

  assert(!dead_nodes[networkNode->clv_index] &&
         !skipped_nodes[networkNode->clv_index]);
  ...
  std::vector<CumulatedChild> cum_children = getCumulatedChildren(
      network, networkParentNode, networkNode, dead_nodes, skipped_nodes);
  assert(cum_children.size() == 2 ||
         (cum_children.size() == 3 &&
          networkParentNode ==
              nullptr));  // 2 children, or started with root node.
  ...
  for (size_t i = 0; i < toChildren.size(); ++i) {
    ...
    connect_subtree_recursive(network, cum_children[i].child, toChildren[i],
                              cum_children[i].direct_parent, dead_nodes,
                              skipped_nodes);
  }
...
std::vector<bool> collect_dead_nodes(Network &network,
  ...
  for (size_t i = 0; i < network.num_reticulations(); ++i) {
    q.emplace(
        getReticulationNonActiveParent(network, network.reticulation_nodes[i]));
  }
  ...
  std::vector<Node *> children =
      getActiveAliveChildren(network, dead_nodes, dtroot);
...
std::vector<bool> collect_skipped_nodes(Network &network,
                                        const std::vector<bool> &dead_nodes) {
  ...
  for (size_t i = 0; i < network.num_nodes(); ++i) {
    ...
    skipped_nodes[i] =
        (getActiveAliveNeighbors(network, dead_nodes, network.nodes_by_index[i])
             .size() == 2);
    assert(dead_nodes[i] || getActiveAliveNeighbors(network, dead_nodes,
...
double displayed_tree_prob(AnnotatedNetwork &ann_network, size_t tree_index) {
  ...
  for (size_t i = 0; i < ann_network.network.num_reticulations(); ++i) {
    ...
    size_t first_parent_pmatrix_idx = getReticulationFirstParentPmatrixIndex(
        ann_network.network.reticulation_nodes[i]);
    double prob = (active_pmatrix_idx == first_parent_pmatrix_idx)
                      ? ann_network.reticulation_probs[i]
                      : 1.0 - ann_network.reticulation_probs[i];
    logProb += log(prob);
  }
  return exp(logProb);
}

Node *getPossibleTreeRootNode(Network &network,
                              const std::vector<bool> &dead_nodes) {
...
pll_utree_t *displayed_tree_to_utree(Network &network, size_t tree_index) {
  ...
  std::vector<bool> dead_nodes =
      collect_dead_nodes(network, network.root->clv_index);
  Node *root = nullptr;
  ...
  pll_unode_t *uroot = connect_subtree_recursive(
      network, root, nullptr, nullptr, dead_nodes, skipped_nodes);
  ...
  for (size_t i = 0; i < utree->inner_count + utree->tip_count; ++i) {
    if (utree->nodes[i]->clv_index < utree->tip_count) {
      ...
      utree->nodes[i]->clv_index = utree->nodes[i]->node_index =
          networkNode->clv_index;
    }
  }
...
pll_utree_t *displayed_tree_to_utree(
    Network &network,
    const std::vector<ReticulationState> &reticulationChoices) {
  for (size_t i = 0; i < reticulationChoices.size();
...
void forbidSubnetwork(Network &network, Node *node,
                      std::vector<bool> &forbidden) {
  if (forbidden[node->clv_index]) return;
...
void getTipVectorRecursive(pll_unode_t *actParent, pll_unode_t *actNode,
                           size_t pmatrix_idx, bool pmatrix_idx_found,
                           std::vector<bool> &res) {
  if (!actNode) {
    return;
  }
  ...
  if (pllmod_utree_is_tip(actNode) && pmatrix_idx_found) {
    ...
    do {
      if (link->back != actParent) {
        getTipVectorRecursive(actNode, link->back, pmatrix_idx,
...
std::vector<bool> getTipVector(const pll_utree_t &utree, size_t pmatrix_idx) {
  ...
  //	starting to write to the tip vector as soon as we have encountered the
  //wanted pmatrix_idx
  getTipVectorRecursive(utree.vroot->back, utree.vroot, pmatrix_idx, false,
                        res);

  // vroot and vroot->back have the same pmatrix index!!!
  if (utree.vroot->pmatrix_index != pmatrix_idx) {
    getTipVectorRecursive(utree.vroot, utree.vroot->back, pmatrix_idx, false,
                          res);
  }
  return res;
}

void getTipVectorRecursive(Network &network, Node *actParent, Node *actNode,
                           size_t pmatrix_idx, bool pmatrix_idx_found,
                           std::vector<bool> &res) {
  ...
  if (actNode->isTip() && pmatrix_idx_found) {
    ...
    std::vector<Node *> activeChildren =
        getActiveChildrenUndirected(network, actNode, actParent);
    for (size_t i = 0; i < activeChildren.size(); ++i) {
...
std::vector<bool> getTipVector(Network &network, size_t pmatrix_idx) {
  ...
  //	starting to write to the tip vector as soon as we have encountered the
  //wanted pmatrix_idx
  getTipVectorRecursive(network, nullptr, network.root, pmatrix_idx, false,
                        res);
  return res;
}

std::vector<std::vector<size_t>> getDtBranchToNetworkBranchMapping(
    const pll_utree_t &utree, Network &network, size_t tree_idx) {
  std::vector<std::vector<size_t>> res(utree.edge_count);
  setReticulationParents(network, tree_idx);

  // for each branch, we need to figure out which tips are on one side of the
  // branch, and which tips are on the other side so essentially, we need to
  // compare bipartitions. That's all!

  // ... we can easily get the set of tips which are in a subtree!
  //  (of either of the endpoints of the current branch, we don't really
  //  care)!!!
...
std::vector<Node *> reversed_topological_sort(Network &network) {
  ...
  while (!q.empty()) {
    ...
    if (actNode->type == NodeType::BASIC_NODE) {
      if (parent[actNode->clv_index] !=
          nullptr) {  // catch special case for root node
