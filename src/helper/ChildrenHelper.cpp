...
std::vector<Node *> getChildren(Network &network, const Node *node) {
  ...
  if (node->type == NodeType::RETICULATION_NODE) {
    ...
    for (size_t i = 0; i < neighbors.size(); ++i) {
      if (getLinksToClvIndex(network, node, neighbors[i]->clv_index)[0]
...
std::vector<Node *> getChildrenIgnoreDirections(Network &network,
                                                const Node *node,
                                                const Node *myParent) {
  assert(node);
  std::vector<Node *> children;
...
std::vector<Node *> getActiveChildren(Network &network, const Node *node) {
  ...
  assert(activeChildren.size() <= 2 ||
         (node == network.root && activeChildren.size() == 3));
  return activeChildren;
}

std::vector<Node *> getActiveAliveChildren(Network &network,
...
std::vector<Node *> getActiveChildrenUndirected(Network &network,
                                                const Node *node,
                                                const Node *myParent) {
  assert(node);
  std::vector<Node *> activeChildren;
  std::vector<Node *> children =
      getChildrenIgnoreDirections(network, node, myParent);
  for (size_t i = 0; i < children.size(); ++i) {
    if (children[i]->getType() == NodeType::RETICULATION_NODE) {
      // we need to check if the child is active, this is, if we are currently
      // the selected parent or we could be the current child...
...
bool hasChild(Network &network, const Node *parent, const Node *candidate) {
  ...
  assert(candidate);
  std::vector<Node *> children = getChildren(network, parent);
  return (std::find(children.begin(), children.end(), candidate) !=
          children.end());
}

std::vector<Node *> getCurrentChildren(
    AnnotatedNetwork &ann_network, const Node *node, const Node *parent,
    const ReticulationConfigSet &restrictions) {
  assert(restrictions.configs.size() == 1);
  std::vector<Node *> children =
      getChildrenIgnoreDirections(ann_network.network, node, parent);
  assert(node->isTip() || !children.empty());
  std::vector<Node *> res;
  for (size_t i = 0; i < children.size(); ++i) {
    if (reticulationConfigsCompatible(
            restrictions,
            getRestrictionsToTakeNeighbor(ann_network, node, children[i]))) {
