...
double rankCandidates(AnnotatedNetwork& ann_network, std::vector<Move>& candidates, bool silent = true, bool print_progress = true) {
    ...
    if (!ann_network.options.no_prefiltering) {
        prefilter_bic = prefilterCandidates(ann_network, candidates, silent, print_progress);
    }
    ...
    if (best_bic > prefilter_bic) {
        throw std::runtime_error("best_bic > prefilter_bic");
    }
...
double chooseCandidate(AnnotatedNetwork& ann_network, std::vector<Move>& candidates, NetworkState* state, bool enforce, bool silent = true, bool print_progress = true) {
    ...
    double rank_bic = rankCandidates(ann_network, candidates, silent, print_progress);
...
int findBestMaxDistance(AnnotatedNetwork& ann_network, MoveType type, const std::vector<MoveType>& typesBySpeed, int step_size, bool silent) { 
...
std::vector<Move> fastIterationsMode(AnnotatedNetwork& ann_network, int best_max_distance, MoveType type, const std::vector<MoveType>& typesBySpeed, double* best_score, BestNetworkData* bestNetworkData, bool silent) {
    ...
    bool rspr1_present = (std::find(typesBySpeed.begin(), typesBySpeed.end(), MoveType::RSPR1Move) != typesBySpeed.end());
    bool delta_plus_present = (std::find(typesBySpeed.begin(), typesBySpeed.end(), MoveType::DeltaPlusMove) != typesBySpeed.end());
    std::vector<Move> candidates = possibleMoves(ann_network, type, rspr1_present, delta_plus_present, 0, best_max_distance);
...
double slowIterationsMode(AnnotatedNetwork& ann_network, MoveType type, int step_size, const std::vector<MoveType>& typesBySpeed, double* best_score, BestNetworkData* bestNetworkData, bool silent) {
    ...
    while (got_better) {
        ...
        got_better = false;
        std::vector<Move> candidates = possibleMoves(ann_network, type, rspr1_present, delta_plus_present, min_dist, max_dist);
