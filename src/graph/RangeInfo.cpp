...
for the tip nodes */
  ...
  size_t inner_clv_num_entries =
      (size_t)sites_alloc * partition->states_padded * partition->rate_cats;

  return ClvRangeInfo{alignment, total_num_clvs, start, end,
                      inner_clv_num_entries};
}

double *create_single_empty_clv(ClvRangeInfo rangeInfo) {
  double *single_clv = (double *)pll_aligned_alloc(
      rangeInfo.inner_clv_num_entries * sizeof(double), rangeInfo.alignment);
  if (!single_clv) {
...
states with vectorized code */
  memset(single_clv, 0, rangeInfo.inner_clv_num_entries * sizeof(double));
  return single_clv;
}
...
unsigned int *create_single_empty_scale_buffer(ScaleBufferRangeInfo rangeInfo) {
  unsigned int *single_scale_buffer =
      (unsigned int *)calloc(rangeInfo.scaler_size, sizeof(unsigned int));
  if (!single_scale_buffer) {
    throw std::runtime_error(
        "Unable to allocate enough memory for scale buffer.");
  }
  return single_scale_buffer;
}

unsigned int **create_empty_scale_buffer(ScaleBufferRangeInfo rangeInfo) {
  unsigned int **scale_buffer = (unsigned int **)calloc(
      rangeInfo.num_scale_buffers, sizeof(unsigned int *));
...
unsigned int *clone_single_scale_buffer(ScaleBufferRangeInfo scaleBufferInfo,
  ...
  unsigned int *cloned_scale_buffer =
      create_single_empty_scale_buffer(scaleBufferInfo);
  memcpy(cloned_scale_buffer, scale_buffer,
         scaleBufferInfo.scaler_size * sizeof(unsigned int));
...
unsigned int **clone_scale_buffer(pll_partition_t *partition,
                                  unsigned int **scale_buffer) {
  unsigned int **cloned_scale_buffer =
      create_empty_scale_buffer(get_scale_buffer_range(partition));
...
void assign_scale_buffer_entries(pll_partition_t *partition,
  ...
  for (unsigned int i = 0; i < rangeInfo.num_scale_buffers; ++i) {
    memcpy(to_scale_buffer[i], from_scale_buffer[i],
