...
namespace netrax {
size_t Network::num_tips() const { return tipCount; }

size_t Network::num_inner() const { return nodeCount - tipCount; }

size_t Network::num_branches() const { return branchCount; }

size_t Network::num_reticulations() const { return reticulation_nodes.size(); }

size_t Network::num_nodes() const { return nodeCount; }
...
void cloneNodesAndEdges(Network &network, const Network &other) {
  for (size_t i = 0; i < other.num_nodes(); ++i) {
    if (other.nodes_by_index[i]->type == NodeType::BASIC_NODE) {
      network.nodes[i].initBasic(other.nodes_by_index[i]->clv_index,
                                 other.nodes_by_index[i]->scaler_index,
                                 other.nodes_by_index[i]->label);
      if (i < other.num_tips()) {
        assert(!other.nodes_by_index[i]->label.empty());
        assert(!network.nodes[i].label.empty());
      }
    } else {  // reticulation node
      ReticulationData *other_ret_data =
          other.nodes_by_index[i]->getReticulationData().get();
      ReticulationData ret_data;
      ret_data.init(other_ret_data->reticulation_index, other_ret_data->label,
                    other_ret_data->active_parent_toggle, nullptr, nullptr,
                    nullptr);
      network.nodes[i].initReticulation(
          i, other.nodes_by_index[i]->scaler_index,
          other.nodes_by_index[i]->label, ret_data);
    }
  }
  for (size_t i = 0; i < other.num_branches(); ++i) {
    network.edges[i].init(i, nullptr, nullptr, other.edges_by_index[i]->length,
                          other.edges_by_index[i]->prob);
  }
}
...
void fillByIndexReferences(Network &network, const Network &other) {
  ...
  for (size_t i = 0; i < other.reticulation_nodes.size(); ++i) {
    network.reticulation_nodes.emplace_back(
        network.nodes_by_index[other.reticulation_nodes[i]->clv_index]);
  }
}
...
void createTheLinks(Network &network, const Network &other) {
  ...
  for (size_t i = 0; i < other.num_nodes(); ++i) {
    for (size_t j = 0; j < other.nodes_by_index[i]->links.size(); ++j) {
      Link link;
      link.init(other.nodes_by_index[i]->links[j].node_clv_index,
                other.nodes_by_index[i]->links[j].edge_pmatrix_index, nullptr,
                nullptr, other.nodes_by_index[i]->links[j].direction);
  ...
  for (size_t i = 0; i < network.nodes.size(); ++i) {
    for (size_t j = 0; j < network.nodes[i].links.size(); ++j) {
      ...
      if (network.nodes[i].links[j].direction == Direction::OUTGOING) {
        ...
        network.edges_by_index[pmatrix_index]->link1 =
            &network.nodes[i].links[j];
      } else {
        assert(!network.edges_by_index[pmatrix_index]->link2);
        network.edges_by_index[pmatrix_index]->link2 =
            &network.nodes[i].links[j];
      }
    }
  }
...
void setOuterLinks(Network &network) {
  for (size_t i = 0; i < network.edges.size(); ++i) {
    ...
    assert(network.edges_by_index[network.edges[i].pmatrix_index] ==
           &network.edges[i]);
...
void setReticulationLinks(Network &network, const Network &other) {
  for (size_t i = 0; i < other.reticulation_nodes.size(); ++i) {
    ReticulationData *other_ret_data =
        other.reticulation_nodes[i]->getReticulationData().get();
    assert(other_ret_data);
    ReticulationData *my_ret_data =
        network.reticulation_nodes[i]->getReticulationData().get();
    assert(my_ret_data);

    size_t first_parent_edge_index =
        other_ret_data->link_to_first_parent->edge_pmatrix_index;
    size_t second_parent_edge_index =
        other_ret_data->link_to_second_parent->edge_pmatrix_index;
    size_t child_edge_index = other_ret_data->link_to_child->edge_pmatrix_index;

    my_ret_data->link_to_first_parent =
        network.edges_by_index[first_parent_edge_index]
            ->link2;  // because it is an incoming link
    my_ret_data->link_to_second_parent =
        network.edges_by_index[second_parent_edge_index]
            ->link2;  // because it is an incoming link
    my_ret_data->link_to_child = network.edges_by_index[child_edge_index]
                                     ->link1;  // because it is an outgoing link
  }
}
...
void applyBranchLengths(Network &network,
...
bool checkSanity(Network &network) {
  ...
  for (size_t i = 0; i < network.num_nodes(); ++i) {
    ...
    for (Link &link : network.nodes_by_index[i]->links) {
      assert(network.edges_by_index[link.edge_pmatrix_index]->link1 == &link ||
             network.edges_by_index[link.edge_pmatrix_index]->link2 == &link);
...
void checkLinkDirections(Network &network) {
  for (size_t i = 0; i < network.num_nodes(); ++i) {
    ...
    for (size_t j = 0; j < network.nodes_by_index[i]->links.size(); ++j) {
      if (network.nodes_by_index[i]->links[j].direction ==
          Direction::OUTGOING) {
        n_out++;
      }
    }
