...
struct Node {
 public:
  ...
  void initReticulation(size_t index, int scaler_index,
  ...
  void setClvIndex(size_t index) { this->clv_index = index; }

  void setScalerIndex(int idx) { scaler_index = idx; }

  Link *getLink() { return &links[0]; }
  ...
  Node(const Node &other) {  // copy constructor
    if (other.type == NodeType::BASIC_NODE) {
      ...
      ret_data.init(other_ret_data->reticulation_index, other_ret_data->label,
                    ...
                    nullptr);
      initReticulation(other.clv_index, other.scaler_index, other.label,
                       ret_data);
    }
