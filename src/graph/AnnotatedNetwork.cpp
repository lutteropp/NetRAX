...
void init_annotated_network(AnnotatedNetwork &ann_network, std::mt19937& rng) {
    ...
    for (size_t i = 0; i < ann_network.pernode_displayed_tree_data.size(); ++i) {
        ann_network.pernode_displayed_tree_data[i].displayed_trees.resize(ann_network.fake_treeinfo->partition_count);
    }
    for (size_t i = 0; i < ann_network.network.num_tips(); ++i) {
        for (size_t p = 0; p < ann_network.fake_treeinfo->partition_count; ++p) {
            ann_network.pernode_displayed_tree_data[i].displayed_trees[p].emplace_back(DisplayedTreeData(ann_network.fake_treeinfo->partitions[p]->clv[i], ann_network.options.max_reticulations));
