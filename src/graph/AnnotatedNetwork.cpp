...
void init_annotated_network(AnnotatedNetwork &ann_network, std::mt19937& rng) {
    ...
    for (size_t p = 0; p < ann_network.fake_treeinfo->partition_count; ++p) {
        ...
        for (size_t i = 0; i < ann_network.network.num_tips(); ++i) { // tip nodes always have valid clv
            ann_network.fake_treeinfo->clv_valid[p][i] = 1;
        }
        for (size_t i = ann_network.network.num_tips(); i < ann_network.network.nodes.size(); ++i) { // nodes.size here, to also take care of currently unused nodes
    ...
    for (size_t i = 0; i < ann_network.network.num_tips(); ++i) {
        ...
        ann_network.pernode_displayed_tree_data[i].displayed_trees.emplace_back(DisplayedTreeData(ann_network.fake_treeinfo, clvInfo, scaleBufferInfo, tip_clvs, ann_network.options.max_reticulations));
