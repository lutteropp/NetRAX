...
void print_model_params(const pll_partition_t &partition) {
  ...
  if (partition.rate_cats > 1) {
    std::vector<double> ratecatRates(partition.rates,
                                     partition.rates + partition.rate_cats);
    print_vector<double>(ratecatRates, "ratecat rates");
    std::vector<double> ratecatWeights(
        partition.rate_weights, partition.rate_weights + partition.rate_cats);
