...
int parseOptions(int argc, char **argv, netrax::NetraxOptions *options) {
  CLI::App app{
      "NetRAX: Phylogenetic Network Inference without Incomplete Lineage "
      "Sorting"};
  app.add_option("--msa", options->msa_file,
                 "The Multiple Sequence Alignment File");
  app.add_option("--model", options->model_file,
                 "The partitions assignment in case of a partitioned MSA.");
  app.add_option("-o,--output", options->output_file,
                 "File where to write the final network to");
  app.add_option(
      "--start_network", options->start_network_file,
  ...
  app.add_flag(
      "-e,--endless", options->endless,
      "Endless search mode - keep trying with more random start networks.");
  app.add_option("--seed", options->seed, "Seed for random number generation.");
  app.add_flag("--score_only", options->score_only,
               "Only read a network and MSA from file and compute its score.");
  app.add_flag("--extract_displayed_trees", options->extract_displayed_trees,
               "Only extract all displayed trees with their probabilities from "
               "a network.");
  app.add_flag(
      "--check_weird_network", options->check_weird_network,
      "Only check if the network has displayed trees with equal bipartitions");
  app.add_flag("--extract_taxon_names", options->extract_taxon_names,
               "Only extract all taxon names from a network.");
  app.add_flag("--generate_random_network_only",
               options->generate_random_network_only,
               "Only generate a random network, with as many reticulations as "
               "specified in the -r parameter");
  app.add_flag("--pretty_print_only", options->pretty_print_only,
               "Only pretty-print a given input network.");
  app.add_option("--scale_branches_only", options->scale_branches_only,
                 "Only scale branches of a given network by a given factor.");

  app.add_flag("--network_distance_only", options->network_distance_only,
               "Only compute unrooted softwired network distance.");
  app.add_option("--first_network", options->first_network_path,
                 "Path to first network file for distance computation.");
  ...
  app.add_option(
      "--overwritten_reticulation_prob", options->overwritten_reticulation_prob,
      "New probability to use for the reticulations in overwrite-only mode.");

  std::string brlen_linkage = "linked";
  app.add_option("--brlen", brlen_linkage,
                 "branch length linkage between partitions (linked, scaled, or "
                 "unlinked) (default: linked)");
  ...
  app.add_option("--prefilter_keep", options->prefilter_keep,
                 "Number of promising candidates to keep after prefiltering "
                 "(default: 60)");
  app.add_option(
  ...
  app.add_flag(
      "--greedy_prefiltering", options->greedy_prefiltering,
  ...
  app.add_option("--max_better_candidates", options->max_better_candidates,
                 "Stop candidate evaluation early if this many candidates with "
                 "better BIC have been found already.");

  app.add_flag("--no_rnni_moves", options->no_rnni_moves,
               "Do not use rNNI moves.");
  app.add_flag("--no_rspr_moves", options->no_rspr_moves,
               "Do not use rSPR moves.");
  app.add_flag("--no_arc_insertion_moves", options->no_arc_insertion_moves,
               "Use only DeltaPlus moves instead of full ArcInsertion moves "
               "(faster, but worse inference quality).");
...
void pretty_print(const NetraxOptions &netraxOptions) {
  Network network = netrax::readNetworkFromFile(
      netraxOptions.start_network_file, netraxOptions);
  if (ParallelContext::master_rank() && ParallelContext::master_thread()) {
    std::cout << exportDebugInfoNetwork(network) << "\n";
  }
}

void score_only(NetraxOptions &netraxOptions, const RaxmlInstance &instance,
                std::mt19937 &rng) {
  ...
  if (ParallelContext::master_rank() && ParallelContext::master_thread()) {
    std::cout << "Initial (before brlen and reticulation opt) BIC Score: "
              << start_bic << "\n";
    std::cout << "Initial (before brlen and reticulation opt) loglikelihood: "
              << start_logl << "\n";
  }
  ...
  if (ParallelContext::master_rank() && ParallelContext::master_thread()) {
    std::cout << "Number of reticulations: "
...
void extract_taxon_names(const NetraxOptions &netraxOptions) {
  netrax::Network network = netrax::readNetworkFromFile(
      netraxOptions.start_network_file, netraxOptions,
...
void extract_displayed_trees(NetraxOptions &netraxOptions,
                             const RaxmlInstance &instance, std::mt19937 &rng) {
  ...
  if (ann_network.network.num_reticulations() == 0) {
    ...
    for (int tree_index = 0;
         tree_index < 1 << ann_network.network.num_reticulations();
         ++tree_index) {
      pll_utree_t *utree =
          netrax::displayed_tree_to_utree(ann_network.network, tree_index);
      double prob = netrax::displayed_tree_prob(ann_network, tree_index);
      Network displayedNetwork =
          netrax::convertUtreeToNetwork(*utree, netraxOptions, 0);
...
void scale_branches_only(NetraxOptions &netraxOptions,
                         const RaxmlInstance &instance, std::mt19937 &rng) {
  netrax::AnnotatedNetwork ann_network =
      build_annotated_network(netraxOptions, instance);
  init_annotated_network(ann_network, rng);
  ...
  for (size_t i = 0; i < ann_network.network.num_branches(); ++i) {
    ann_network.network.edges_by_index[i]->length *=
        netraxOptions.scale_branches_only;
...
void network_distance_only(NetraxOptions &netraxOptions,
  ...
  netrax::AnnotatedNetwork ann_network_1 = build_annotated_network_from_file(
      netraxOptions, instance, netraxOptions.first_network_path);
  init_annotated_network(ann_network_1, rng);
  netrax::AnnotatedNetwork ann_network_2 = build_annotated_network_from_file(
      netraxOptions, instance, netraxOptions.second_network_path);
  ...
  if (ParallelContext::master_rank() && ParallelContext::master_thread()) {
    std::cout << "Unrooted softwired network distance: "
              << get_network_distance(
                     ann_network_1, ann_network_2, label_to_int,
...
void check_weird_network(NetraxOptions &netraxOptions,
                         const RaxmlInstance &instance, std::mt19937 &rng) {
  ...
  for (size_t i = 0; i < displayed_trees.size(); ++i) {
    for (size_t j = i + 1; j < displayed_trees.size(); ++j) {
      ...
      unsigned int rf_dist = pllmod_utree_rf_distance(
          displayed_trees[i]->vroot, displayed_trees[j]->vroot, n_tips);
      if (rf_dist == 0) {
        n_equal++;
      }
    }
...
void generate_random_network_only(NetraxOptions &netraxOptions,
                                  const RaxmlInstance &instance,
                                  std::mt19937 &rng) {
  std::uniform_int_distribution<long> dist(0, RAND_MAX);
  netrax::AnnotatedNetwork ann_network =
      build_random_annotated_network(netraxOptions, instance, dist(rng));
  init_annotated_network(ann_network, rng);
  ann_network.fake_treeinfo->brlen_linkage = PLLMOD_COMMON_BRLEN_LINKED;
...
void scale_reticulation_probs_only(NetraxOptions &netraxOptions,
                                   const RaxmlInstance &instance,
                                   std::mt19937 &rng) {
  netrax::AnnotatedNetwork ann_network =
      build_annotated_network(netraxOptions, instance);
...
bool quick_function(NetraxOptions &netraxOptions,
                    const RaxmlInstance &instance) {
  ...
  if (netraxOptions.pretty_print_only) {
    ...
    if (ParallelContext::master_rank() &&
        ParallelContext::master_thread()) {  // only the master rank does the
                                             // simple work
    ...
    if (netraxOptions.overwritten_reticulation_prob < 0.0 ||
        netraxOptions.overwritten_reticulation_prob > 1.0) {
    ...
    if (netraxOptions.first_network_path.empty() ||
        netraxOptions.second_network_path.empty()) {
...
void netrax_thread_main(NetraxOptions &netraxOptions,
                        const RaxmlInstance &instance) {
...
void setup_parallel_stuff(NetraxOptions &netraxOptions,
  ...
  auto thread_function = std::bind(netrax_thread_main, std::ref(netraxOptions),
                                   std::ref(instance));
  ...
  if (instance.opts.msa_format == FileFormat::binary &&
      instance.opts.use_rba_partload) {
...
int internal_main_netrax(int argc, char **argv, void *comm) {
  ...
  // netrax::Network nw =
  // netrax::readNetworkFromString("(((12:0.0381986,(((14:0.185353,(((((13:1.42035e-06)#0:1.43322e-06::0.5)#2:1.31229e-06::0.5)#3:1.4605e-06::0.5)#4:1.34252e-06::0.5)#5:0.0625::0.5):0.0926765)#1:0.0463383::0.5,#3:1::0.5):0.0463383):1.33647e-06,((((10:0.0445575,5:0.100001):1e-06,(3:0.140615,#4:1::0.5):0.140615):1e-06,#5:1::0.5):1e-06,#1:0.449939::0.5):1e-06):1e-06,11:0.0328376,(9:0.0343774,(#0:0.0935504::0.5,#2:1::0.5):0.0935504):1.33647e-06);");
