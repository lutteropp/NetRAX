...
int parseOptions(int argc, char **argv, netrax::NetraxOptions *options)
{
    ...
    app.add_option("--prefilter_fraction", options->prefilter_fraction, "Fraction of prefiltered candidates to keep (default: 0.1)");
    ...
    app.add_option("--scrambling", options->scrambling, "Maximum failed consecutive scrambling retries for escaping out of local maxima (default: 10).");
...
bool quick_function(NetraxOptions& netraxOptions, const RaxmlInstance& instance) {
    ...
    if (netraxOptions.pretty_print_only) {
        ...
        if (ParallelContext::local_proc_id() == 0) { // only the master rank does the simple work
