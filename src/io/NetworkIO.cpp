...
Network convertNetworkToplevel(RootedNetwork &rnetwork, size_t node_count,
  ...
  if (maxReticulations >= 0 &&
      (unsigned int)maxReticulations < rnetwork.reticulationCount) {
    throw std::runtime_error(
        "number of reticulations in the network is higher than maximum "
        "reticulation count");
  }
  unsigned int maxExtraReticulations =
      (maxReticulations != -1) ? maxReticulations - rnetwork.reticulationCount
                               : 0;
  ...
  std::vector<RootedNetworkNode *> rnetwork_tips, rnetwork_inner_tree,
      rnetwork_reticulations;
  ...
  for (RootedNetworkNode *rnode : rnetwork_tips) {
    ...
    network.edges[pmatrix_index].init(pmatrix_index, nullptr, nullptr,
                                      rnode->length);

    Link *linkToParent =
        make_link(&network.nodes[clv_index], &network.edges[pmatrix_index],
                  Direction::INCOMING);
  ...
  for (const auto &rnode : rnetwork_nodes) {
    ...
    if (rnode->isReticulation) {  // 2 parents
      size_t pmatrix_index = rnetwork_tips.size() + rnetwork_inner_tree.size() -
                             1 + 2 * rnode->reticulation_index;

      assert(rnode->firstParent->clv_index < network.num_nodes());
      assert(rnode->secondParent->clv_index < network.num_nodes());
      assert(pmatrix_index + 1 < network.num_branches());

      Link *linkFromFirstParent =
          make_link(&network.nodes[rnode->firstParent->clv_index],
                    &network.edges[pmatrix_index], Direction::OUTGOING);
      Link *linkFromSecondParent =
          make_link(&network.nodes[rnode->secondParent->clv_index],
      ...
      Link *linkFromParent =
          make_link(&network.nodes[rnode->parent->clv_index],
                    &network.edges[pmatrix_index], Direction::OUTGOING);
    ...
    if (rnode->isReticulation) {  // 2 parents
      ...
      if (rnode->firstParent->clv_index == rnode->secondParent->clv_index) {
        std::vector<Link *> links =
            getLinksToClvIndexMutable(network, &network.nodes[rnode->clv_index],
      ...
      network.nodes[rnode->clv_index]
          .getReticulationData()
          ->link_to_first_parent = linkToFirstParent;
      network.nodes[rnode->clv_index]
          .getReticulationData()
          ->link_to_second_parent = linkToSecondParent;
      ...
      network.nodes[rnode->clv_index].getReticulationData()->link_to_child =
          linkToChild;
    } else {  // 1 parent
      size_t pmatrix_index = rnode->clv_index;
  ...
  // change links from edges such that link1 is the outgoing link and link2 is
  // the incoming link
  for (size_t i = 0; i < network.num_branches(); ++i) {
    if (network.edges_by_index[i]->link1->direction == Direction::INCOMING) {
      std::swap(network.edges_by_index[i]->link1,
                network.edges_by_index[i]->link2);
    }
  }
...
std::pair<size_t, size_t> makeToplevel(RootedNetwork &rnetwork) {
  ...
  // special case: check if rnetwork.root has only one child... if so, reset the
  // root to its child.
...
std::string newickNodeName(Network &network, const Node *node,
                           const Node *parent) {
  assert(node);
  std::stringstream sb("");
...
std::string printNodeNewick(Network &network, Node *node, Node *parent,
  ...
  if (!children.empty() &&
    ...
    sb << printNodeNewick(network, children[children.size() - 1], node,
...
void collect_average_branches(AnnotatedNetwork &ann_network) {
  // If we have unlinked branch lenghts/probs, replace the entries in the
  // network by their average
  assert(ann_network.fake_treeinfo->brlen_linkage ==
         PLLMOD_COMMON_BRLEN_UNLINKED);
  std::vector<std::vector<double>> partition_brlens(
      ann_network.fake_treeinfo->partition_count,
      std::vector<double>(ann_network.network.num_branches(), 0.0));
  ...
  if (ann_network.fake_treeinfo->parallel_reduce_cb) {
    ann_network.fake_treeinfo->parallel_reduce_cb(
        ...
        ann_network.fake_treeinfo->linked_branch_lengths,
        ann_network.network.num_branches(), PLLMOD_COMMON_REDUCE_MAX);
  }

  memset(ann_network.fake_treeinfo->linked_branch_lengths, 0,
...
void updateNetwork(AnnotatedNetwork &ann_network) {
  ...
  for (size_t i = 0; i < ann_network.network.num_reticulations(); ++i) {
    size_t pmatrix_index = getReticulationFirstParentPmatrixIndex(
        ann_network.network.reticulation_nodes[i]);
    size_t contra_pmatrix_index = getReticulationSecondParentPmatrixIndex(
        ann_network.network.reticulation_nodes[i]);
    ann_network.network.edges_by_index[pmatrix_index]->prob =
        ann_network.reticulation_probs[i];
    ann_network.network.edges_by_index[contra_pmatrix_index]->prob =
...
std::string toExtendedNewick(Network &network) {
  std::unordered_set<Node *> visited_reticulations;
  return printNodeNewick(network, network.root, nullptr,
                         visited_reticulations) +
         ";";
}
