  ...
  for (unsigned int n = 0; n < sites; ++n) {
    for (unsigned int i = 0; i < rate_cats; ++i) {
      for (unsigned int j = 0; j < states; ++j) {
        std::cout << clv[j + i * states_padded] << "\n";
      }
    }
  }
}
...
void print_clv_vector(pllmod_treeinfo_t &fake_treeinfo, size_t tree_idx,
  ...
  std::cout << "clv vector for tree_idx " << tree_idx << " and clv_index "
            << clv_index << ":\n";
...
void printReticulationParents(Network &network) {
  ...
  for (size_t i = 0; i < network.num_reticulations(); ++i) {
    std::cout
        << "  reticulation " << network.reticulation_nodes[i]->clv_index
        << " has parent "
  ...
  if (ann_network.options.brlen_linkage == PLLMOD_COMMON_BRLEN_UNLINKED) {
    for (size_t p = 0; p < ann_network.fake_treeinfo->partition_count; ++p) {
      for (size_t i = 0; i < ann_network.network.num_branches(); ++i) {
        std::cout << "brlens[" << p << "][" << i
                  << "]: " << ann_network.fake_treeinfo->branch_lengths[p][i]
                  << "\n";
      }
    }
  } else {
...
void printClvValid(AnnotatedNetwork &ann_network) {
  for (size_t i = 0; i < ann_network.network.num_nodes(); ++i) {
    std::cout << "clv_valid[" << ann_network.network.nodes[i].clv_index
              << "] = "
              << (int)ann_network.fake_treeinfo
                     ->clv_valid[0][ann_network.network.nodes[i].clv_index]
              << "\n";
  }
  std::cout << "\n";
}
...
void printReticulationFirstParents(AnnotatedNetwork &ann_network) {
  ...
  for (Node *node : ann_network.network.reticulation_nodes) {
    std::cout
        << "  ret node " << node->clv_index << " has first parent "
...
std::string exportDebugInfoRootedNetwork(const RootedNetwork &rnetwork) {
  ...
  for (size_t i = 0; i < rnetwork.nodes.size(); ++i) {
    ss << "\tnode\n\t[\n\t\tid\t" << i << "\n";
    std::string nodeLabel = (rnetwork.nodes[i]->label.empty())
                                ? std::to_string(i)
                                : rnetwork.nodes[i]->label;
    ss << "\t\tlabel\t\"" << nodeLabel << "\"\n";
  ...
  for (size_t i = 0; i < rnetwork.nodes.size(); ++i) {
    for (RootedNetworkNode *child : rnetwork.nodes[i]->children) {
      ...
      unsigned int childId = std::distance(
          rnetwork.nodes.begin(),
...
std::string exportDebugInfoExtraNodeNumber(
    Network &network, const std::vector<unsigned int> &extra_node_number,
    bool with_label) {
  std::stringstream ss;
  ss << "graph\n[\tdirected\t1\n";
  ...
  for (size_t i = 0; i < network.nodes.size(); ++i) {
    ...
    if (network.nodes_by_index[i]->getType() == NodeType::RETICULATION_NODE) {
      size_t firstParentIdx = network.nodes_by_index[i]
                                  ->getReticulationData()
                                  ->getLinkToFirstParent()
                                  ->outer->node_clv_index;
      nodeLabel +=
          "(r=" +
          std::to_string((network.nodes_by_index[i]->getReticulationData())
                             ->getReticulationIndex()) +
          ", p0=" + std::to_string(firstParentIdx) + ")";
    }
...
std::string exportDebugInfo(AnnotatedNetwork &ann_network, bool with_label) {
  ...
  return exportDebugInfoExtraNodeNumber(
      ann_network.network, std::vector<unsigned int>(), with_label);
}
...
void print_partition(AnnotatedNetwork &ann_network,
  ...
  for (size_t clv_index = 0; clv_index < ann_network.network.num_nodes();
       ++clv_index) {
    pll_show_clv(partition, clv_index,
                 ann_network.network.nodes_by_index[clv_index]->scaler_index,
                 10);
  }

  std::cout << "pmatrix:\n";
  for (size_t pmatrix_index = 0;
       pmatrix_index < ann_network.network.num_branches(); ++pmatrix_index) {
...
void print_treeinfo(AnnotatedNetwork &ann_network) {
  ...
  std::cout << "default_likelihood_computation_params: "
            << treeinfo->default_likelihood_computation_params << "\n";
  std::cout << "default_likelihood_target_function: "
            << treeinfo->default_likelihood_target_function << "\n";
  std::cout << "init_partition_count: " << treeinfo->init_partition_count
            << "\n";
  std::cout << "likelihood_target_function: "
            << treeinfo->likelihood_target_function << "\n";
  ...
  // std::cout << "likelihood_computation_params: " <<
  // treeinfo->likelihood_computation_params << "\n"; std::cout << "tree: " <<
  // treeinfo->tree << "\n";
        ...
        DisplayedTreeData &tree =
            ann_network
                .pernode_displayed_tree_data[ann_network.network.root
                                                 ->clv_index]
                .displayed_trees[j];
        pll_utree_t *utree = netrax::displayed_tree_to_utree(
            ann_network.network,
            tree.treeLoglData.reticulationChoices.configs[0]);
        double prob = std::exp(tree.treeLoglData.tree_logprob);
        Network displayedNetwork =
            netrax::convertUtreeToNetwork(*utree, ann_network.options, 0);
...
void printDisplayedTreesChoices(AnnotatedNetwork &ann_network,
                                Node *virtualRoot) {
  NodeDisplayedTreeData &nodeData =
      ann_network.pernode_displayed_tree_data[virtualRoot->clv_index];
  for (size_t i = 0; i < nodeData.num_active_displayed_trees; ++i) {
    printReticulationChoices(
        nodeData.displayed_trees[i].treeLoglData.reticulationChoices);
  }
}
...
void printAllDisplayedTreeConfigs(AnnotatedNetwork &ann_network) {
  for (size_t i = 0; i < ann_network.network.num_nodes(); ++i) {
    std::cout << "displayed trees at node " << i << ":"
              << "\n";
    NodeDisplayedTreeData &nodeData =
        ann_network.pernode_displayed_tree_data[i];
    for (size_t j = 0; j < nodeData.num_active_displayed_trees; ++j) {
      ...
      if (!nodeData.displayed_trees[j].treeLoglData.tree_logprob_valid) {
        nodeData.displayed_trees[j].treeLoglData.tree_logprob =
      ...
      std::cout << " tree prob: "
                << exp(nodeData.displayed_trees[j].treeLoglData.tree_logprob)
                << "\n";
    }
  }
}
