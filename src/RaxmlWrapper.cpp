...
double network_logl_wrapper(void *network_params, int incremental,
  ...
  return computeLoglikelihood(*params->ann_network, incremental,
                              update_pmatrices);
}
double network_opt_brlen_wrapper(pllmod_treeinfo_t *fake_treeinfo,
                                 double min_brlen, double max_brlen,
                                 double lh_epsilon, int max_iters,
                                 int opt_method, int radius) {
  AnnotatedNetwork *ann_network =
      ((NetworkParams *)(fake_treeinfo->likelihood_computation_params))
          ->ann_network;
  return optimize_branches(*ann_network, max_iters, max_iters, radius);
}
double network_spr_round_wrapper(
    pllmod_treeinfo_t *treeinfo, unsigned int radius_min,
...
int pllmod_treeinfo_init_partition_sarah(pllmod_treeinfo_t *treeinfo,
                                         ...
                                         int params_to_optimize, int gamma_mode,
                                         double alpha,
  ...
  memset(treeinfo->pmatrix_valid[partition_index], 0,
         pmatrix_count * sizeof(char));
  ...
  if (subst_matrix_symmetries) {
    ...
    memcpy(treeinfo->subst_matrix_symmetries[partition_index],
           subst_matrix_symmetries, symm_size);
  } else
  ...
  memset(treeinfo->deriv_precomp[partition_index], 0,
         precomp_size * sizeof(double));

  return PLL_SUCCESS;
}
void set_partition_fake_clv_entry(pll_partition_t *partition,
                                  size_t fake_clv_index) {
  ...
  if (clv == NULL) {  // this happens when we have site repeats
    // TODO: Does it work? Or do we need to increase the number of tips somehow
    // when creating the partition?
    partition->clv[fake_clv_index] = (double *)pll_aligned_alloc(
        sites_alloc * rate_cats * states_padded * sizeof(double),
        partition->alignment);
...
void network_create_init_partition_wrapper(
    size_t p, int params_to_optimize, pllmod_treeinfo_t *pll_treeinfo,
    const Options &opts, const PartitionInfo &pinfo,
    const IDVector &tip_msa_idmap,
    PartitionAssignment::const_iterator &part_range,
    const uintVector &weights) {
  /* create and init PLL partition structure */
  pll_partition_t *partition = create_pll_partition(
      opts, pinfo, tip_msa_idmap, *part_range, weights,
      pll_treeinfo->tree->tip_count, pll_treeinfo->tree->inner_count,
      pll_treeinfo->tree->edge_count);
  int retval = pllmod_treeinfo_init_partition_sarah(
      pll_treeinfo, p, partition, params_to_optimize,
      pinfo.model().gamma_mode(), pinfo.model().alpha(),
...
void network_init_treeinfo_wrapper(
    const Options &opts, const std::vector<doubleVector> &partition_brlens,
    size_t num_branches, const PartitionedMSA &parted_msa,
  ...
  // init partitions
  int optimize_branches =
      opts.optimize_brlen ? PLLMOD_OPT_PARAM_BRANCHES_ITERATIVE : 0;

  for (size_t p = 0; p < parted_msa.part_count(); ++p) {
    const PartitionInfo &pinfo = parted_msa.part_info(p);
    const auto &weights =
        site_weights.empty() ? pinfo.msa().weights() : site_weights.at(p);
    int params_to_optimize =
        opts.optimize_model ? pinfo.model().params_to_optimize() : 0;
    params_to_optimize |= optimize_branches;

    (*partition_contributions)[p] =
    ...
    if (part_range != part_assign.end()) {
      ...
      pll_partition_t *partition = create_pll_partition(
          opts, pinfo, tip_msa_idmap, *part_range, weights);
      int retval = pllmod_treeinfo_init_partition(
          pll_treeinfo, p, partition, params_to_optimize,
          pinfo.model().gamma_mode(), pinfo.model().alpha(),
          pinfo.model().ratecat_submodels().data(),
          pinfo.model().submodel(0).rate_sym().data());
      ...
      if (part_range->master()) (*parts_master).insert(p);
    } else {
      // this partition will be processed by other threads, but we still need to
      // know which parameters to optimize
      pll_treeinfo->params_to_optimize[p] = params_to_optimize;
    }
  }

  // finalize partition contribution computation
  for (auto &c : *partition_contributions) c /= total_weight;
}
...
void reset_tip_ids(
  ...
  // We leave the edges and nodes arrays as they are, and only change the index
  // fields of their entries
...
Options createDefaultOptions() {
  Options opts;
  /* if no command specified, default to --search (or --help if no args were
   * given) */
...
void setupRaxmlInstanceSingleThreaded(RaxmlInstance &instance) {
  ...
  instance.proc_part_assign =
      instance.load_balancer->get_all_assignments(part_sizes, 1);
  return;
}
  ...
  if (instance.parted_msa->part_count() == 1) {
    if (options.brlen_linkage != PLLMOD_COMMON_BRLEN_LINKED) {
      throw std::runtime_error(
          "Only one partition given, but brlen linkage is not set to linked");
    }
...
TreeInfo::tinfo_behaviour createNetworkBehaviour() {
  ...
  network_behaviour.compute_ancestral_function =
      [&](pllmod_treeinfo_t *treeinfo) {
        return network_ancestral_wrapper(treeinfo);
      };
  network_behaviour.opt_brlen_function = [&](pllmod_treeinfo_t *fake_treeinfo,
                                             double min_brlen, double max_brlen,
                                             double lh_epsilon, int max_iters,
                                             int opt_method, int radius) {
    return network_opt_brlen_wrapper(fake_treeinfo, min_brlen, max_brlen,
                                     lh_epsilon, max_iters, opt_method, radius);
  };
  network_behaviour.spr_round_function =
      [&](pllmod_treeinfo_t *treeinfo, unsigned int radius_min,
          unsigned int radius_max, unsigned int ntopol_keep,
          pll_bool_t thorough, int brlen_opt_method, double bl_min,
          double bl_max, int smoothings, double epsilon,
          cutoff_info_t *cutoff_info, double subtree_cutoff) {
        return network_spr_round_wrapper(
            treeinfo, radius_min, radius_max, ntopol_keep, thorough,
            brlen_opt_method, bl_min, bl_max, smoothings, epsilon, cutoff_info,
            subtree_cutoff);
      };
  network_behaviour.destroy_treeinfo_function =
      [&](pllmod_treeinfo_t *treeinfo) {
        return destroy_network_treeinfo(treeinfo);
      };
  network_behaviour.create_init_partition_function =
      network_create_init_partition_wrapper;
...
int fake_init_tree(pllmod_treeinfo_t *treeinfo, Network &network) {
  ...
  tree->inner_count = network.nodes.size() - network.num_tips() +
                      1;  // +1 for the fake clv index
...
void fake_init_collect_branch_lengths(pllmod_treeinfo_t *treeinfo,
  ...
  for (size_t i = 0; i < network.edges.size() + 1;
       ++i) {  // +1 for the fake branch length
...
pllmod_treeinfo_t *createNetworkPllTreeinfoInternal(
    AnnotatedNetwork &ann_network, unsigned int tips,
  ...
  for (size_t p = 0; p < n_p; ++p) {
    for (size_t i = 0; i < ann_network.network.num_branches(); ++i) {
      treeinfo->branch_lengths[p][i] = std::min(treeinfo->branch_lengths[p][i],
                                                ann_network.options.brlen_max);
      treeinfo->branch_lengths[p][i] = std::max(treeinfo->branch_lengths[p][i],
                                                ann_network.options.brlen_min);
    }
  }

  assert(static_cast<NetworkParams *>(treeinfo->likelihood_computation_params)
...
pllmod_treeinfo_t *createNetworkPllTreeinfo(AnnotatedNetwork &ann_network) {
  ...
  pllmod_treeinfo_t *pllTreeinfo = createNetworkPllTreeinfoInternal(
      ann_network, network.num_tips(), instance);
  ...
  ann_network.total_num_model_parameters =
      instance.parted_msa->total_free_model_params();
...
std::cout << "local proc id: " << ParallelContext::local_proc_id() << "\n";
...
std::cout << "local group id: " << ParallelContext::local_group_id() << "\n";*/
  ...
  const std::vector<uintVector> site_weights =
      std::vector<uintVector>();  // they are only relevant for bootstrapping
  ...
  for (size_t p = 0; p < parted_msa.part_count(); ++p) {
    ...
    ann_network.partition_contributions[p] =
        std::accumulate(weights.begin(), weights.end(), 0);
    ...
    if (part_range != part_assign.end()) {
      /* create and init PLL partition structure */
      network_create_init_partition_wrapper(p, params_to_optimize, pllTreeinfo,
                                            opts, pinfo, tip_msa_idmap,
                                            part_range, weights);
  ...
  // finalize partition contribution computation
  for (auto &c : ann_network.partition_contributions) c /= total_weight;
...
TreeInfo *createRaxmlTreeinfo(pllmod_treeinfo_t *treeinfo,
  ...
  const PartitionAssignment &part_assign =
      instance.proc_part_assign.at(ParallelContext::proc_id());
  return new TreeInfo(instance.opts, std::vector<doubleVector>(), treeinfo,
                      (*instance.parted_msa.get()), instance.tip_msa_idmap,
                      part_assign, std::vector<uintVector>(), behaviour);
}
...
pllmod_treeinfo_t *createStandardPllTreeinfo(const pll_utree_t *utree,
                                             ...
                                             int brlen_linkage) {
  return pllmod_treeinfo_create(utree->vroot, utree->tip_count, partitions,
                                brlen_linkage);
}

TreeInfo *createRaxmlTreeinfo(pll_utree_t *utree,
                              const RaxmlInstance &instance) {
  // Check that the MSA has already been loaded
  assert(!instance.tip_id_map.empty());
  reset_tip_ids(utree, instance.tip_id_map);
  pllmod_treeinfo_t *pllTreeinfo = createStandardPllTreeinfo(
      utree, instance.parted_msa->part_count(), instance.opts.brlen_linkage);
...
TreeInfo *createRaxmlTreeinfo(pll_utree_t *utree, const RaxmlInstance &instance,
  ...
  TreeInfo::tinfo_behaviour standard_behaviour;
  TreeInfo *info =
      createRaxmlTreeinfo(pllTreeinfo, instance, standard_behaviour);
