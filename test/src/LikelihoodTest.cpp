...
TEST_F(LikelihoodTest, testTheTest) { EXPECT_TRUE(true); }
...
std::unordered_set<std::string> collect_tip_labels_utree(pll_utree_t *utree) {
  ...
  pll_utree_traverse(utree->vroot, PLL_TREE_TRAVERSE_POSTORDER, cb_trav_all,
                     outbuffer.data(), &trav_size);
...
void comparePartitions(const pll_partition_t *p_network,
                       const pll_partition_t *p_raxml) {
  ...
  unsigned int start =
      (p_raxml->attributes & PLL_ATTRIB_PATTERN_TIP) ? p_raxml->tips : 0;
  unsigned int end = p_raxml->tips + p_raxml->clv_buffers;

  size_t sites_alloc =
      (unsigned int)p_raxml->asc_additional_sites + p_raxml->sites;
...
pll_unode_t *getNodeWithClvIndex(unsigned int clv_index,
...
bool isLeafNode(const pll_unode_t *node) { return (node->next == NULL); }

void compareLikelihoodFunctions(const std::string &networkPath,
  ...
  ASSERT_NE(norep_logl_blobs_graycode,
            -std::numeric_limits<double>::infinity());
  double naive_logl =
      computeLoglikelihoodNaiveUtree(ann_network, 0, 1, &treewise_logl_naive);
...
void incrementalTest(const std::string &networkPath,
                     const std::string &msaPath) {
  NetraxOptions options;
  options.run_single_threaded = true;
  options.start_network_file = networkPath;
  options.msa_file = msaPath;
...
TEST_F(LikelihoodTest, smallNetworkIncremental) {
  incrementalTest(DATA_PATH + "small.nw",
                  DATA_PATH + "small_fake_alignment.txt");
}

TEST_F(LikelihoodTest, celineNetworkIncremental) {
  incrementalTest(DATA_PATH + "celine.nw",
                  DATA_PATH + "celine_fake_alignment.txt");
}

TEST_F(LikelihoodTest, smallNetwork) {
  compareLikelihoodFunctions(DATA_PATH + "small.nw",
                             DATA_PATH + "small_fake_alignment.txt", false);
}

TEST_F(LikelihoodTest, smallTree) {
  compareLikelihoodFunctions(DATA_PATH + "tree.nw",
                             DATA_PATH + "small_fake_alignment.txt", false);
}

TEST_F(LikelihoodTest, smallTreeIncremental) {
  incrementalTest(DATA_PATH + "tree.nw",
                  DATA_PATH + "small_fake_alignment.txt");
}

TEST_F(LikelihoodTest, tinyNetwork) {
  compareLikelihoodFunctions(DATA_PATH + "tiny.nw",
                             DATA_PATH + "tiny_fake_alignment.txt", false);
}
...
TEST_F(LikelihoodTest, smallNetworkWithRepeats) {
  compareLikelihoodFunctions(DATA_PATH + "small.nw",
                             DATA_PATH + "small_fake_alignment.txt", true);
}
...
TEST_F(LikelihoodTest, celineNetworkSmaller) {
  compareLikelihoodFunctions(DATA_PATH + "celine_smaller_1.nw",
                             DATA_PATH + "celine_fake_alignment_smaller.txt",
                             false);
}
...
TEST_F(LikelihoodTest, DISABLED_displayedTreeOfNetworkToUtree) {
  ...
  // compare tip labels
  std::unordered_set<std::string> tip_labels_utree =
      collect_tip_labels_utree(utree);
  EXPECT_EQ(tip_labels_utree.size(), smallNetwork.num_tips());
  for (size_t i = 0; i < smallNetwork.num_tips(); ++i) {
    EXPECT_TRUE(tip_labels_utree.find(smallNetwork.nodes[i].label) !=
                tip_labels_utree.end());
  }
  // compare tip labels for second tree
  std::unordered_set<std::string> tip_labels_utree2 =
      collect_tip_labels_utree(utree2);
  EXPECT_EQ(tip_labels_utree2.size(), smallNetwork.num_tips());
  for (size_t i = 0; i < smallNetwork.num_tips(); ++i) {
    EXPECT_TRUE(tip_labels_utree2.find(smallNetwork.nodes[i].label) !=
                tip_labels_utree2.end());
  }
...
TEST_F(LikelihoodTest, convertUtreeToNetwork) {
  ...
  AnnotatedNetwork ann_network =
      build_annotated_network_from_utree(options, instance, *raxml_utree);
  init_annotated_network(ann_network);
  Network &network = ann_network.network;
  print_clv_index_by_label(network);
  ...
  double naive_network_logl =
      computeLoglikelihoodNaiveUtree(ann_network_2, 0, 1);
