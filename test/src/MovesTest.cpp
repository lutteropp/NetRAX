...
void randomMovesStep(AnnotatedNetwork &ann_network,
                     std::vector<Move> candidates) {
  ASSERT_FLOAT_EQ(netrax::computeLoglikelihood(ann_network, 1, 1),
  ...
  for (size_t j = 0; j < candidates.size(); ++j) {
    std::cout << "Testing moves for candidate " << j + 1 << "/"
              << candidates.size() << "...\n";
    ...
    ASSERT_DOUBLE_EQ(netrax::computeLoglikelihood(ann_network, 1, 1),
                     netrax::computeLoglikelihood(ann_network, 0, 1));
    ...
    ASSERT_EQ(origMove.arcRemovalData.a_clv_index,
              candidates[j].arcRemovalData.a_clv_index);
    ASSERT_EQ(origMove.arcRemovalData.b_clv_index,
              candidates[j].arcRemovalData.b_clv_index);
    ASSERT_EQ(origMove.arcRemovalData.c_clv_index,
              candidates[j].arcRemovalData.c_clv_index);
    ASSERT_EQ(origMove.arcRemovalData.d_clv_index,
              candidates[j].arcRemovalData.d_clv_index);
    ASSERT_EQ(origMove.arcRemovalData.u_clv_index,
              candidates[j].arcRemovalData.u_clv_index);
    ASSERT_EQ(origMove.arcRemovalData.v_clv_index,
              candidates[j].arcRemovalData.v_clv_index);
    ASSERT_EQ(origMove.arcRemovalData.au_pmatrix_index,
              candidates[j].arcRemovalData.au_pmatrix_index);
    ASSERT_EQ(origMove.arcRemovalData.cv_pmatrix_index,
              candidates[j].arcRemovalData.cv_pmatrix_index);
    ASSERT_EQ(origMove.arcRemovalData.ub_pmatrix_index,
              candidates[j].arcRemovalData.ub_pmatrix_index);
    ASSERT_EQ(origMove.arcRemovalData.uv_pmatrix_index,
              candidates[j].arcRemovalData.uv_pmatrix_index);
    ASSERT_EQ(origMove.arcRemovalData.vd_pmatrix_index,
              candidates[j].arcRemovalData.vd_pmatrix_index);

    ASSERT_EQ(origMove.arcInsertionData.a_clv_index,
              candidates[j].arcInsertionData.a_clv_index);
    ASSERT_EQ(origMove.arcInsertionData.b_clv_index,
              candidates[j].arcInsertionData.b_clv_index);
    ASSERT_EQ(origMove.arcInsertionData.c_clv_index,
              candidates[j].arcInsertionData.c_clv_index);
    ASSERT_EQ(origMove.arcInsertionData.d_clv_index,
              candidates[j].arcInsertionData.d_clv_index);
    ASSERT_EQ(origMove.arcInsertionData.ab_pmatrix_index,
              candidates[j].arcInsertionData.ab_pmatrix_index);
    ASSERT_EQ(origMove.arcInsertionData.cd_pmatrix_index,
              candidates[j].arcInsertionData.cd_pmatrix_index);

    ASSERT_EQ(origMove.rsprData.x_clv_index,
              candidates[j].rsprData.x_clv_index);
    ASSERT_EQ(origMove.rsprData.y_clv_index,
              candidates[j].rsprData.y_clv_index);
    ASSERT_EQ(origMove.rsprData.z_clv_index,
              candidates[j].rsprData.z_clv_index);
    ASSERT_EQ(origMove.rsprData.x_prime_clv_index,
              candidates[j].rsprData.x_prime_clv_index);
    ASSERT_EQ(origMove.rsprData.y_prime_clv_index,
              candidates[j].rsprData.y_prime_clv_index);

    ASSERT_EQ(origMove.rnniData.u_clv_index,
              candidates[j].rnniData.u_clv_index);
    ASSERT_EQ(origMove.rnniData.v_clv_index,
              candidates[j].rnniData.v_clv_index);
    ASSERT_EQ(origMove.rnniData.s_clv_index,
              candidates[j].rnniData.s_clv_index);
    ASSERT_EQ(origMove.rnniData.t_clv_index,
              candidates[j].rnniData.t_clv_index);
...
void twoMoves(const std::string &networkPath, const std::string &msaPath,
              bool useRepeats, MoveType type) {
  NetraxOptions options;
  options.run_single_threaded = true;
  options.start_network_file = networkPath;
...
void insertAndRemove(const std::string &networkPath, const std::string &msaPath,
                     bool useRepeats) {
  NetraxOptions options;
  options.run_single_threaded = true;
  options.start_network_file = networkPath;
  ...
  std::vector<Move> insertionCandidates =
      possibleMoves(ann_network, MoveType::ArcInsertionMove);
  for (size_t i = 0; i < 10; ++i) {
    performMove(ann_network, insertionCandidates[0]);
    std::vector<Move> removalCandidates =
        possibleMoves(ann_network, MoveType::ArcRemovalMove);
