...
TEST(BrlenOptTest, testTheTest) { ASSERT_TRUE(true); }

TEST(BrlenOptTest, treeVirtualRootProblem) {
  // initial setup
  std::string treePath =
      DATA_PATH +
      "0_4_taxa_1_reticulations_CELINE_PERFECT_SAMPLING_200_msasize_1_0_"
      "brlenScaler_0_5_reticulation_prob.raxml.bestTree";
  std::string msaPath = DATA_PATH +
                        "0_4_taxa_1_reticulations_CELINE_PERFECT_SAMPLING_200_"
                        "msasize_1_0_brlenScaler_0_5_reticulation_prob_msa.txt";
  std::string partitionsPath =
      DATA_PATH +
      "0_4_taxa_1_reticulations_CELINE_PERFECT_SAMPLING_200_msasize_1_0_"
      "brlenScaler_0_5_reticulation_prob_partitions.txt";
  ...
  std::cout << "  true logl old brlen, partition 0: "
            << ann_network.fake_treeinfo->partition_loglh[0] << "\n";
  std::cout << "  true logl old brlen, partition 1: "
            << ann_network.fake_treeinfo->partition_loglh[1] << "\n";

  auto oldTrees = extractOldTrees(ann_network, ann_network.network.root);
  std::uniform_int_distribution<std::mt19937::result_type> dist(
      0, ann_network.network.num_branches() - 1);

  ReticulationConfigSet restrictions =
      getRestrictionsActiveBranch(ann_network, pmatrix_index);
  updateCLVsVirtualRerootTrees(ann_network, old_virtual_root, new_virtual_root,
                               new_virtual_root_back, restrictions);
  ...
  double old_logl_reroot = computeLoglikelihoodBrlenOpt(ann_network, oldTrees,
                                                        edge->pmatrix_index, 1);

  std::cout << "reroot logl old brlen: " << old_logl_reroot << "\n";
  std::cout << "  reroot logl old brlen, partition 0: "
            << ann_network.fake_treeinfo->partition_loglh[0] << "\n";
  std::cout << "  reroot logl old brlen, partition 1: "
            << ann_network.fake_treeinfo->partition_loglh[1] << "\n";
  ann_network.fake_treeinfo->linked_branch_lengths[pmatrix_index] =
      0.049539358966596169775442604077397845685482025146484375;
  ...
  std::cout << "  true logl new brlen, partition 0: "
            << ann_network.fake_treeinfo->partition_loglh[0] << "\n";
  std::cout << "  true logl new brlen, partition 1: "
            << ann_network.fake_treeinfo->partition_loglh[1] << "\n";
  updateCLVsVirtualRerootTrees(
      ann_network, ann_network.network.root,
      ...
      restrictions);
  ann_network.cached_logl_valid = false;
  std::cout << "reroot logl new brlen, after computing true logl new brlen: "
            << computeLoglikelihoodBrlenOpt(ann_network, oldTrees,
                                            pmatrix_index)
            << "\n";
  std::cout << "  reroot logl new brlen, after computing true logl new brlen, "
               "partition 0: "
  ...
  ann_network.cached_logl_valid = false;
  std::cout << "reroot logl old brlen here: "
            << computeLoglikelihoodBrlenOpt(ann_network, oldTrees,
                                            pmatrix_index)
            << "\n";
  std::cout << "  reroot logl old brlen here, partition 0: "
            << ann_network.fake_treeinfo->partition_loglh[0] << "\n";
  std::cout << "  reroot logl old brlen here, partition 1: "
            << ann_network.fake_treeinfo->partition_loglh[1] << "\n";
  invalidatePmatrixIndex(ann_network, pmatrix_index);
  ...
  std::cout << "true logl old brlen again: " << true_logl_old_brlen_again
            << "\n";
  std::cout << "  true logl old brlen again, partition 0: "
            << ann_network.fake_treeinfo->partition_loglh[0] << "\n";
  std::cout << "  true logl old brlen again, partition 1: "
            << ann_network.fake_treeinfo->partition_loglh[1] << "\n";
...
TEST(BrlenOptTest, treeVirtualRoots) {
  ...
  std::string msaPath = DATA_PATH +
                        "0_4_taxa_1_reticulations_CELINE_PERFECT_SAMPLING_200_"
                        "msasize_1_0_brlenScaler_0_5_reticulation_prob_msa.txt";
  // std::string partitionsPath = DATA_PATH +
  // "0_4_taxa_1_reticulations_CELINE_PERFECT_SAMPLING_200_msasize_1_0_brlenScaler_0_5_reticulation_prob_partitions.txt";
  NetraxOptions options;
  options.run_single_threaded = true;
  options.start_network_file = treePath;
  options.msa_file = msaPath;
  ...
  std::uniform_int_distribution<std::mt19937::result_type> dist(
      0, annTreeNetwork.network.num_branches() - 1);
  for (size_t i = 0; i < 100; ++i) {
    ...
    std::cout << "new_virtual_root_back: " << new_virtual_root_back->clv_index
              << "\n";
    ReticulationConfigSet restrictions =
        getRestrictionsActiveBranch(annTreeNetwork, pmatrix_index);
    updateCLVsVirtualRerootTrees(annTreeNetwork, old_virtual_root,
    ...
    annTreeNetwork.cached_logl_valid = false;
    double new_logl = computeLoglikelihoodBrlenOpt(annTreeNetwork, oldTrees,
                                                   edge->pmatrix_index, 1);
    ...
    // change the brlen
    double old_brlen =
        annTreeNetwork.fake_treeinfo->linked_branch_lengths[pmatrix_index];
    annTreeNetwork.fake_treeinfo->linked_branch_lengths[pmatrix_index] = 12345;
    for (size_t p = 0; p < annTreeNetwork.fake_treeinfo->partition_count; ++p) {
      annTreeNetwork.fake_treeinfo->pmatrix_valid[p][pmatrix_index] = 0;
    }
    annTreeNetwork.cached_logl_valid = false;
    double mid_logl = computeLoglikelihoodBrlenOpt(annTreeNetwork, oldTrees,
                                                   edge->pmatrix_index, 1);
    ...
    annTreeNetwork.cached_logl_valid = false;
    double recomputed_logl = computeLoglikelihoodBrlenOpt(
        annTreeNetwork, oldTrees, edge->pmatrix_index, 1);
...
TEST(BrlenOptTest, smallVirtualRoots) {
  ...
  for (size_t p = 0; p < annTreeNetwork.fake_treeinfo->partition_count; ++p) {
    size_t n_trees =
        annTreeNetwork
            .pernode_displayed_tree_data[annTreeNetwork.network.root->clv_index]
            .num_active_displayed_trees;
    for (size_t i = 0; i < n_trees; ++i) {
      DisplayedTreeData &tree =
          annTreeNetwork
              .pernode_displayed_tree_data[annTreeNetwork.network.root
                                               ->clv_index]
              .displayed_trees[i];
      std::cout << "correct partition " << p << ", tree " << i
                << " logl: " << tree.treeLoglData.tree_partition_logl[p]
                << ", logprob: " << tree.treeLoglData.tree_logprob << "\n";
    }
  }
