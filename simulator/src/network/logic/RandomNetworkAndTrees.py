...
class SimulationParameters:
    def __init__(self):
        ...
        self.inheritance = True # to print inheritance probs (Dendroscope cannot read them, so we print also a network without them
...
def Newick_From_MULTree(params, tree,root,hybrid_nodes):
    ...
    for v in tree.successors(root):
        if not(params.ILS):
            Newick+= Newick_From_MULTree(params,tree,v,hybrid_nodes)+":"+str(tree[root][v]['length'])
        else:
            coalescent_time = tree[root][v]['length']/params.pop_size
            Newick+= Newick_From_MULTree(params,tree,v,hybrid_nodes)+":"+str(coalescent_time)
