...
WANTED_MSA_SIZES = [1000, 5000, 10000]
# wanted values for m. Used to generate m*2^k trees with k being the number of reticulations in the network.
WANTED_M_VALUES = [1, 5, 10, 20, 30]
WANTED_SIMULATORS = [SimulationType.SARAH]  # [SimulationType.CELINE]
WANTED_SAMPLINGS = [SamplingType.STANDARD, SamplingType.PERFECT_SAMPLING,
                    SamplingType.PERFECT_UNIFORM_SAMPLING, SamplingType.SINGLE_SITE_SAMPLING]
WANTED_LIKELIHOODS = [LikelihoodType.AVERAGE, LikelihoodType.BEST]

# Timeout of zero means a single netrax random starting network, timeout larger than zero means continue starting from other random networks until timeout has been reached
WANTED_TIMEOUTS = [0, 60]


def build_dataset(n_taxa, n_reticulations, msa_size, m, simulator, sampling, likelihood, timeout):
    if not os.path.exists('datasets'):
        os.makedirs('datasets')
    name = "datasets/" + str(simulator) + "_" + str(sampling) + "_" + str(likelihood) + "_" + str(
        n_taxa) + "_" + str(n_reticulations) + "_" + str(msa_size) + "_" + str(m) + "_" + str(timeout)
                            ...
                            for likelihood in wanted_likelihoods:
                                for timeout in wanted_timeouts:
                                    my_dataset = build_dataset(
                                        n_taxa, n_reticulations, msa_size, m, simulator, sampling, likelihood, timeout)
...
if __name__ == "__main__":
    run_experiments(WANTED_NUM_TAXA, WANTED_NUM_RETICULATIONS, WANTED_MSA_SIZES, WANTED_M_VALUES,
                    WANTED_SIMULATORS, WANTED_SAMPLINGS, WANTED_LIKELIHOODS, WANTED_TIMEOUTS)
